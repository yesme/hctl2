...
> 状态：Q1–Q3 与 C1 均已由所有者于 2026-09-19 确认；本页所列四项无需重拍，后续体验补充单独记录。<br>
> 日期：2026-09-19<br>
...
**后续工作：** 按此对齐[新建流程 P1](./02-user-journey.md#p1选择代码来源)和 Repo 注册路径的规范与实现；本轮只更新体验文档。
...
| 范围 | 现文位置 | 要对齐的内容 |
| --- | --- | --- |
| 工作边界与命名 | [三面架构](../design/architecture.md#单元与连接)、[设计地图](../design/README.md#对象关系)、[Project 约束](../design/spec/project.md#对象)、[Repo 约束](../design/spec/repo.md#repo-注册) | 落实 Q1：Project 作为工作范围，Repo 保留代码职责；不把 Topic 当旧 Project；独立控制面可使用同一外部 Repo |
...
| Room 分类与入口 | [Project 正文](../design/project.md#room-类型)、[Room 约束](../design/spec/project.md#room-与消息) | 唯一 Project Room 与 Topic Rooms；新 Topic Room 以前情提要接续主 Room，参与者不必通读原聊天；Scoped Room 用途并入；实际请求仍有处理路径，不给所有 Topic 强加结案手续 |
...
| 多 Source | [Task 正文](../design/task.md#kanban-场景)、[来源约束](../design/spec/task.md#契约与来源) | 每 Source 一个入口；新 Task 目标 Source 明确；任务来源固定与源内状态、排序变化分开；对齐旧合并板、旧 Project 分组及唯一源引用 |
| Run 关联与导航 | [Run 约束](../design/spec/run.md#对象)、[Run 正文](../design/run.md#workflow-场景) | 独立活动列表、默认 DAG、任务书与共同 Worker 侧栏；执行、检查、评审、合入与 Task 验收分别可见，异常时保留原因与处理入口；引用不自动承担完整 Task 交付，不擅自扩大基数或并发规则 |
| 持续建议、模板、观察 | [交付文档](../design/delivery.md#当前范围)、[Participant 正文](../design/participant.md#terminal-场景) | 对齐真实能力与验收；不把未配置归纳、只读 Source、没有图形能力冒充完整覆盖 |
| 术语与验证 | [术语表](../design/references/glossary.md#核心产品词)、[旧 S1 映射](../design/scenarios/S1-multi-unit.md#四不变量)、[契约测试](../design/contract-tests.md) | 术语首字母大写；旧“两个 Project”改成正确工作层次；约束有变更时一起更新版本和可失败的用例 |
...
- 本目录已记录：四份体验正文、第一手出处、09-19 确认的组织结构，以及 Q1–Q3 和 C1 的结论；本页四项均已定。随后按所有者要求补写前情提要与 Run 体验供查看；待处理标记、面板内容、处理后行为与降噪规则也已确认。
