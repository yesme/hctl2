...
- **组织层**：每张卡、每间房、每次 Run 都能按实际来源打开来源链——从讨论采纳的卡由哪段讨论采纳、直接从 Source 认领的卡从哪次认领起、Topic Room 从哪份前情提要开始、随 Project 创建的主 Room 从创建起、这次 Run 冻结了哪份清单——从「这个承诺为什么没完成」追到「哪个执行、哪个版本、哪个检查在阻塞」。用户能查看每一步安排的来源与依据；不为直接认领的卡或主 Room 补造讨论或前情提要。
