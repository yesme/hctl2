...
> 状态：保留溯源；本目录及直接相关的设计、约束、场景与 CT 已同步，待本 PR 复审确认后再判退役，不提前删历史。<br>
> 日期：2026-09-19<br>
...
| 现行设计正文、spec、术语表的领域定义与 CT | 本 PR 同步直接受影响的定义与失败用例，版本为 v0.18.7；保留同 Repo 多 Project、独立授权及原执行规则，不把未实现能力报告为已交付 |
...
所有者允许：所有混用修好后，confession 可以 deprecated 或 removed。本 PR 扩大同步范围后，仍待复审核对以下条件，不由作者提前宣布完成。
