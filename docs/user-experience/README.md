...
> 整理日期：2026-09-19<br>
> 对照基线：整理起点 main @ `41594b714cd7df596df605f8fce2b55750319571`（v0.18.6）；#257 的约束与验收同步版本为 v0.18.7。
...
- 本目录记录需求，不定义数据库对象、状态机或协议。对应约束、设计正文的直接引用与验收要求已随 #257 同步；产品代码与行为验收不因此自动完成。
...
历史原话、文件名、命令和代码标识符保持原样；例如 `mac_ctl` 不改成另一种拼写。提到设计对象时以当前基线（v0.18.7）为准；追溯更早版本时标明版本号并逐项核对异同，不把 Project 整个词或同 Repo 多 Project 的关系标成过时。
