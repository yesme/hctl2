...
> 状态：用户体验基线；Q1–Q3、C1/C2 已确认，Q1 的归纳已按 C2 纠正为同 Control、同 Repo 可有多个 Project；对应约束与场景验收随 #257 同步，不是实现完成报告。<br>
> 整理日期：2026-09-19<br>
> 对照基线：整理起点 main @ `41594b714cd7df596df605f8fce2b55750319571`（v0.18.6）；本 PR 的约束与验收同步版本为 v0.18.7。
...
- 本目录记录需求，不定义数据库对象、状态机或协议。对应约束、设计正文的直接引用与验收要求已随本 PR 同步；产品代码与行为验收不因此自动完成。
- 精确行为要求查[设计约束](../design/spec/README.md#文件)，是否实现查交付证据；约束已对齐不等于实现已经支持。
- 原始材料保留溯源；当前体验只在本目录维护。[S1](../design/scenarios/S1-multi-unit.md#参考用例-s1多单元协作)保留多单元的验证映射，[S3](../design/scenarios/S3-user-journey.md)覆盖本轮新增路径；不另养混用术语的用户叙事。
...
| 所有者 09-19 确认按讨论整体修订本目录、要求术语首字母大写，随后同意 Q2/Q3 并确认 C1 是主 Repo Room | 本目录六份文件对齐；Q1–Q3 与 C1 均记为已定；随后经所有者同意，正式约束与场景验收也并入 #257 |
...
| 所有者 09-19 明确两个 Project 是独立 Namespace，可以各有对应同一外部卡的 Task | [04 留原话](./04-project-navigation.md#2026-09-19project-独立-namespace)；实体到 Task 的唯一范围改为 Project，保留外部卡共享、承诺各自验收 |
| 所有者 09-19：“对，#257 扩为 ‘体验澄清＋场景与验收同步’” | 同 PR 修正直接冲突的约束与设计引用，更新 S1/S2、增加 S3 和既有 CT 族的失败用例，版本推进 v0.18.7；不写产品实现、不扩大已定执行语义 |

## 从另一台机器接手

在自己的工作分支更新到 origin/main，从本页进入：先读 04 确认组织结构，再走 01、02，最后读已定事项与接手清单。03 只在需要追溯旧说法时读；跨机使用新修订前先确认它已合入 main。

当前已记录体验与组织方向，并同步直接相关的设计正文、spec、术语表、场景与契约测试要求；仍待复审、合入以及产品实现和行为测试。接手以仓内记录为依据，不直接按某份旧 Memo 的推荐开工，也不依赖某台机器的 Harness Session 或临时文件。
