...
CLI 没有隐藏权限，也不直接写控制面存储、执行面 content 服务器或 Agency。`terminal attach` 只建立观察或输入通道，不恢复任何领域对象。Run 的语义恢复或替换使用 `run resume|replace`；Room Invocation 的再次施工使用创建新 Invocation 与新代次的 `invocation retry`。终端重连只恢复观察或输入通道，不得改变 Run 或 Room Invocation 的生命周期状态。
...
1. 从 Project 提炼 Task；一个未参加讨论的 Participant 把施工图按三张清单读回成人话，人对照后批准 Workflow Revision 和 Engine Deployment。
...
自举期间每次 Run 从执行事件重建尝试内部的依赖图，记关键路径、总 token、节点数与归约次数，作观测存起来，只看不用，为将来是否放开施工图归约积累数据。自举期间还统计每条门的触发次数与绕过次数，作为验收的一部分：连续若干个真实变更里从未触发或全部被绕过的门，要重新审视是修还是删，删要进台账。任一逃生口（无契约卡、无 Run 路径、一票评审）走完，控制面存储里仍有可回溯的对象与凭证。将来重写 control 或 Workbench，每条被拿掉的门单独写台账行，不允许「不在这版里」。
...
约束层留给施工图或施工清单声明、但要有缺省值的几项，缺省值定在这里，不进约束：Gate 返工轮数上限默认 2 轮（文献里两轮之后自纠开始变差，停滞阈值也取 2，见[多模型协作有效性调研](../research/multi-agent-effectiveness-20260908.md)）；Run 过渡态（启动中、暂停中、取消中）墙钟超时默认 15 分钟；增量评审默认关闭（全量重评）；施工图批准前的盲读回默认开启，施工图可声明关闭；席位多样性策略默认不声明，只靠计票去重兜底。改缺省值只改本节。
