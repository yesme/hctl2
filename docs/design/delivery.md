...
- 同时完成 Linear 与本地任务服务器两套完整任务后端双向适配器；任务源之间的迁移（换卡）不做——卡不搬家；Project 拆分与合并不另造命令——归档旧的、新建新的、来源引用连历史已够；Task 依赖不另造治理对象——按任务源原生语义投影（见 [Task 约束](./spec/task.md#契约与来源)）；
