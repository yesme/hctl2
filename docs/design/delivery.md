...
自举期间统计每条门的触发次数与绕过次数，作为验收的一部分：连续若干个真实变更里从未触发或全部被绕过的门，要重新审视是修还是删，删要进台账。任一逃生口（无契约卡、无 Run 路径、一票评审）走完，控制面存储里仍有可回溯的对象与凭证。将来重写 control 或 Workbench，每条被拿掉的门单独写台账行，不允许「不在这版里」。
...
约束层留给施工图或施工清单声明、但要有缺省值的几项，缺省值定在这里，不进约束：Gate 返工轮数上限默认 2 轮（文献里两轮之后自纠开始变差，停滞阈值也取 2，见[多 agent 协作有效性](../research/multi-agent-effectiveness-20260908.md)）；Run 过渡态（启动中、暂停中、取消中）墙钟超时默认 15 分钟；增量评审默认关闭（全量重评）；施工图批准前的盲读回默认开启，施工图可声明关闭；席位多样性策略默认不声明，只靠计票去重兜底。改缺省值只改本节。
