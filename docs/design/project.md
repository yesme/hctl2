...
- 塑形的三张清单是施工图的来料：只有「已决」里能写成「交出什么、凭什么算交出」的条目才能成为节点候选，「尚未定形」只能成为 Request 或 Scoped Room，「出界」不进图；问题之间的依赖在清单里就写成边。怎么凝结见 [Run 正文](./run.md#施工图怎么凝结)。
...
在 Workbench 里同时管理多个仓库时，一个 Room 可以把另一个仓库 Room 的 Participant 阵容借用为预填选择，不必逐个重选。借用只是预填：规划者仍在本 Room 重新选入，权限、预算和绑定不跨仓库继承；将来若要沉淀为可共享的一等对象，再另行设计。
...
- 注册仓库时 Repo 模块激活仓库身份，Project 在同一账本事务里建 Repo Room；Project 归属于一个仓库。
