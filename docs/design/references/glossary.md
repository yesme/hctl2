...
| Agency | 派出方 | Terminal 场景按冻结规格派出执行体、持有进程/PTY/会话的供给侧受控端口；第一阶段由 Herdr 实现，未来官方远程 Agent 走同一合同 | [spec/agent](../spec/agent.md#运行时与观测) |
...
| Terminal | harness（编码代理工具）/ Agency（派出方；第一阶段为 Herdr） | 会话转录、PTY 流 |

权威定义见[三面架构](../architecture.md#场景与系统)。同根词辨析：Agent（模块）≠ Agency（派出方）≠ agent（执行体的口语说法）。第一阶段由 Herdr 直接实现 Agency，不另设第一方终端运行服务；`provider` 只泛指四个模块的供应端，不是一个跨模块对象或统一接口。
