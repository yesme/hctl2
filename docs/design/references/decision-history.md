...
| v0.17.3 | 2026-09-10 | 三组研究落约束层：Gate 策略加席位多样性字段，计票去重（Worker Profile revision 与 Context Bundle 摘要均相同只计一票）总是生效，席位包不含他席裁决与作者标识，分歧不设商量回合；Profile 规则四——节点判据二选一、边引用上游结晶或凭证、无环且落地、批准后节点集不变；「批准 Workflow」前置读回记录（可声明关闭）；Gate Receipt 逐票记配置、包摘要、去重标记、证据等级与 token；Manifest 冻结项相应扩展；CT-RUN 与 CT-PRODUCT 补七条失败用例 | [裁决记录](../../../.memo/design/design-review-20260902/21-r2-rulings.md) |
| v0.17.2 | 2026-09-10 | 三组研究落设计正文（治理功能消亡 33 家、多 agent 协作有效性、Prove2Me 依赖图）：愿景补「中段判断门无人守住」的行业证据与「省」的标尺、自举是门在工作的唯一证明；Run 正文重写评审独立性（多样性三层、人设不算、同配置同包只算一票、分歧不设商量回合）并新增「施工图怎么凝结」（节点判据二选一、边由机械物承载、盲读回默认开、运行中不归约子节点）；Participant 正文收窄专业化说法（Skill 改上下文、人设是界面属性）；Context、Project、Task 正文各补一句；交付默认值返工上限改 2 轮、自举加门触发计数与逃生口验收；不改约束语义 | [裁决记录](../../../.memo/design/design-review-20260902/21-r2-rulings.md) |
