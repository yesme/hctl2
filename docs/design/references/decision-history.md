...
> 状态：Informative · 对应草案 v0.18.4 · 2026-09-17<br>
...
| v0.18.4 | 2026-09-17 | 交付文档未决问题裁决（所有者 2026-09-17 逐条）：删「ChangeSet 默认基数」（前提「多 Task Run」自 v0.7 起已不存在；Task 与 Run 一对多、Run 与 ChangeSet 与评审请求一对一，写进 spec/repo 发布评审）与「原生会话导入」（非所有者需求，源自 08-14 观察清单，设计正文整节删）；Task 依赖按任务源原生语义投影（父卡、子卡、阻塞它的、它阻塞的四个只读字段）、不另存、改关系走 content 通道、只有阻塞进启动预览等人确认；Scoped Room 不自动归档、闲置进需要关注（默认 14 天）；预算耗尽不再派、跑完当前派工；Project 拆合不另造命令；安全策略原则进 CONSTRAINTS，Repo Room 隐私与远程认证两条随安全批落点 | [#243](https://github.com/yesme/hctl2/pull/243) |
