...
2026-09-06 所有者拍板。§35 给「没有平台的仓库怎么办」的答案是一条不经平台的本地路径：讨论在 Scoped Room，检查是本地测试事实，合入由工具箱推进本地分支。所有者判定这不够：Repo 模块要在本地也完整可用，评审请求、评审线程、保护规则、合入记录这一整套过程必须有承载者。裁决是随包带一个本地代码协作平台，选型 Gitea——小、单二进制、功能全——给只在本地的仓库用；Forgejo 记录在案备选。
