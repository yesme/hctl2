...
> 状态：Informative · 对应草案 v0.18.11 · 2026-09-19<br>
...
Workbench 随后被明确为四个 Scene 的集成客户端，而不是新的领域层。自建它的理由不是重写通用 UI，而是四模块的导航结构无法无损套入任何 donor 的会话、终端或工作树主导航。CLI、Workbench 和适配后的第三方 Scene UI 都通过同一类 Query、Preview、Submit、Subscribe 约束工作；provider 原生 UI 若只操作本系统 content，仍只是 content 客户端或诊断面。关闭任何客户端不改变领域事实，也不授予额外权限。
...
早期实践曾试图通过 prompt 要求 Harness 在“确实完成”时才报告完成，并期待模型自行维持 Task、Run 与执行结果之间的边界。实际使用表明，这种约束不能稳定提供终结权：模型输出仍是受上下文影响的建议，Harness 也只能观察本次执行，无法替 Task 的冻结验收约束作出权威裁决。由此形成一条更一般的教训：凡是能够由身份、版本、状态与证据机械判断的事项，就不再交给 LLM 灵活解释。

这轮改判把 Task 完成收窄为两条可审计路径：有权 human actor 提交「完成 Task」命令，或绑定 Workflow 正常完成后由 reducer 提交同一命令；同时拒绝让 Run 终态直接改写 Task。现行验收、取消与失败处理见 [Task 约束的写入约束](../spec/task.md#写入约束)。
...
这轮改判把协作边收窄为两类：有权 human actor 创建普通 Chat Room 的临场边，reducer 按冻结的 Workflow Revision 创建执行边；Agent 只建议下一位 Participant。现行预览、授权与执行边规则见 [Project 约束的场景约束](../spec/project.md#场景约束)。
...
HCTL2 对 HCTL1 的继承是约束层的，而不是表结构层的：
...
与此同时，对象含义发生了变化。HCTL1 的 `Seat = harness × model`；HCTL2 的 Seat 是 Obligation 内稳定的逻辑执行者或投票位置，并允许多个 Attempt。HCTL1 的 Obligation 主要承载静态 author/gate/merge 责任；HCTL2 的 Obligation 对应一次 Engine external task 的逻辑产出责任。HCTL1 以 Seat refs、PR 和 squash Receipt 作为主要协调事实；在 v0.8/v0.9 当时的设计里，HCTL2 把四模块操作账本放入 Repo Instance SQLite，把共享定义和内容证据放入 Git，把机械工作流位置交给 Conductor，并通过连接约束交换精确引用。
...
这些实现也提供 UI、协议、恢复和运维原语，但都不定义 HCTL2 的公共对象、状态机或事实源。采用原则始终是“保留可复核能力，重画权威边界”：参考实现证明某个局部机制可行，四模块与连接约束决定它在 HCTL2 中能拥有什么。

## 11. 过度精简的教训与愿景层的恢复

v0.8.0 把规范收敛为四个模块时，以“移除平行权威”为由删除了大部分愿景表述：一句话定位、失败模式叙事、四阶段心智模型、目标体验旅程、命名设计原则和各模块的“为什么存在”。约束收紧本身是对的，但这次删除混淆了两类内容：权威去重针对的是**约束**——一个对象只能有一个定义；它不适用于**解释**——愿景、论证和体验叙事不与约束竞争权威，删掉它们只会让约束失去裁决新问题的方向，也让新读者只知道规则是什么、不知道规则为什么存在。

v0.9.0 因此把“为什么”与“是什么”分开维护：愿景、原则和目标体验回到独立的[愿景文档](../vision.md)，四个模块文档各自恢复“为什么存在”的开篇，术语表恢复为非规范对照；全部约束保持 v0.8.1 收紧后的形态不变。四阶段链“意图 → 承诺 → 治理 → 运行”作为心智模型恢复，同时明确一件事不必完整经历四个阶段——用一句话消解线性误读，而不是删除整个模型。

v0.9.1 处理概念层面的同类问题。全量清点暴露出 75 个以上具名概念平铺在设计正文里——很多是执行路径上的一个步骤、一份冻结字段组或一个状态值，被顺手建成了“类”。修订做了三件事：设计正文与约束层分离（`docs/design/spec/`，设计层只用核心产品词加日常语言）；概念按六族归类并归并（复合名 41 → 28，如派发规格、物理运行时、外部副作用意图各归一名）；场景概念对齐外部标准（Chat ↔ Matrix/Slack，Kanban ↔ Linear/GitHub，Workflow ↔ Conductor/BPMN，Terminal ↔ PTY/tmux/ACP），自造词只保留差异化语义。这轮方法论的完整记录见 `.memo/notes/design-doc-method-20260819.md`。
...
这不是把治理交给平台，也不是回到 HCTL1 的“Git 承载一切”：判决仍只在 metadata 层产生，冻结摘要仍是 content 与治理之间的防火墙（第 9 节的继承表原样成立）。v0.10.1 对落点做了一处修正：Board 从 Project 级上移为 Repo 级——一个 Repo 一个 Board，Project 是板上的分组实体，Task 是卡片；这让 GitHub issues 这类天然 repo 级的后端直接对齐，也让支持父子任务的本地服务器以“任务–子任务”承载 Project–Task。v0.10.2 进一步取消了仓库侧的独立物理账本：现场记账本可由 metadata 账本、Git 与运行时观测推导，单立一本账是把实现细节写进约束——clone 本地从此只有 OS 锁与可丢弃缓存，实例注册与现场记账并入用户级账本，存储从“两本账 + Git”收敛为“一本账 + Git”。另有一条词汇裁决随本次转向生效：自 v0.10.0 起，“Agent”一词专属第四模块（原 Harness 模块更名），“Harness”专指编码代理工具这一系统角色，散文中的 AI 协作者用 Participant 表述。类别的权威定义见[约束层总则](../spec/README.md#三类数据)；候选系统与限时验证见[交付文档](../delivery.md)。
...
同轮拍板 chat server 产品方向为 Tuwunel（接口更 API 化、与 Synapse 参考实现兼容性更强；单二进制预编译包、运维压力低；AppService 注册程序化），Continuwuity 记录在案备选；精确发布版本、存储后端与 build features 仍由 P0 验证后固定。该轮同时完成的词形收敛（驼峰名、`*Intent` 命令名与状态值拼写）见[小修订台账](#小修订台账)。
...
ChangeSet 保留原形（核心产品词、业界成词）；字段与格式名（`port_kind`、`review_subject_digest`、ReviewSubjectRef 等）在约束需要逐字指认时保留原形，不受自造语义名的词形规则约束。
...
- **账本备份集约束**：备份必须是唯一 writer 协调的一致快照（含各层 generation 与账本引用的不可变定义字节），恢复保留账本身份、推进全部代次、令旧凭证失效。把既有的“账本必须备份”从一句话落成约束。

方法论记录：这五项随审计一次合并落地，未按「真语义变更单独立项」拆批——同根问题跨文件归并可取，但代价是回滚粒度整体化、新机制险些没有决策记录（本节即补记）。后续审计若再引入新机制，机制本身应单独立项提交。

## 17. 适配器诚实约束：终局结果、观测截断与派生谱系（v0.12.0 补充）

Agent 模块的观测约束原本回答的是“信号如何仲裁”（优先级阶梯、观测不推进领域结果），但留着两个物理层必须表态的空洞：进程干净退出算什么？上报中断后的残缺事件流还算不算历史？本轮据 LobeHub 源码审计（[E-LOBEHUB](../../research/workbench/lobehub.md#e-lobehub)，其 heterogeneous-agents 适配器层的经验证做法）补齐三条 harness 适配器义务：
...
一轮按最新方法论（三类数据、概念门槛）对 README 与设计/约束骨架的复查，解决了六件事：
...
- **概念清扫**：Room Event 除名、Task Operational State 降级为操作投影、执行身份无法证明的终态统一为“丢失”（处理规则唯一定义在连接约束的失败表）；裁决与去向见[小修订台账](#小修订台账)与[核销表](#v0122-清扫)。
...
| 状态值"中断"（Room Invocation） | 统一为"丢失"：执行身份无法证明时 Room Invocation 与 Attempt 进入同一状态；结束规则只在[连接约束](../spec/connections.md#失败与恢复)定义一次 |

## 21. Context 约束裁决轮（v0.12.3）

按"逐条裁决、不整篇升格"把 Context 横切正文已拍板的设计落进约束（Project 模块约束的 Context 一节与交付验证），全轮零新增具名对象：
...
- **压缩约束**：缺省关闭；small-brain 是经用户级定义机制固定 revision/digest 的模型引用（无新对象）；逐条记录 compressor、压缩率与原文 digest 且片段可回源；证据类内容永不压缩，违规 Bundle 拒绝交付；萃取/压缩产物可按（room、cursor 区间、消费者范围）作派生缓存跨调用复用。
...
| Room 的“加密/降级”状态 | 不设：房间端到端加密状态是 Chat 端口绑定的 health 投影，不进不可变 binding，也不是 Room 的 lifecycle 值；可观察结果只在[连接约束失败表](../spec/connections.md#失败与恢复)登记一行，恢复动作是既有的「换绑」命令 |
...
落点：[Context 横切正文](../context.md)新增投喂三档与 Run 内接力两节；[Project 约束](../spec/project.md#context-memo-artifact)记录投喂档、指针可达范围与评论线来源；[Run 约束](../spec/run.md#request重试与-gate)记录 Seat Bundle 的接力内容；[Task 约束](../spec/task.md#契约与来源)记录评论线快照冻结。
...
逐条落点：discovery 默认本地，联网探测可配置且不静默安装改配（系统边界）；危险动作默认 Preview 确认，普通命令可直接 Submit（系统边界）；不可证静默的旧 worktree/ChangeSet 默认隔离，human 确认后可接管、封存、采用或丢弃（Agent 约束）；清理默认保全，确认后可丢弃（Agent 约束）；单写者只保留三条底线——一个逻辑 writer、已确认副作用不重复、旧结果不覆盖新结果，锁路径与推进机制降为实现细节（系统边界）；存储拓扑与 Git 内部命名空间用法降为实现选择（系统边界）；Repo 挂接证据缺失或冲突时展示证据、由用户确认归属（系统边界）；Project 归档只被非终态 Run/写入型 Invocation、活动租约与未决外部写副作用阻塞，开放 Task/Request/Scoped Room 随归档转只读（Project 约束）；Scoped Room 可显式以 abandoned/no-decision/superseded 结案归档（Project 约束）；Context 萃取与相关性门缺省全本地零模型，配置 small-brain 后可模型辅助（Project 约束与横切正文）；活动 Run 期间可采纳新 Task Revision，Run 只按冻结 Revision 完成、current 前移时按分歧拒绝（Task 约束）。
...
## 35. Repo 立为第五模块，代码协作平台显式进设计（v0.17.0）

2026-09-06 所有者拍板。起因是 P1 收口时的一个观察：我们自己的开发流程——开 PR 不合、各家评审留评论、作者修、再审、合入——事实上把协作放在 GitHub 上，而设计既没有写下这个依赖，也没有回答没有平台的仓库怎么办。核查结论是：Git 是基础依赖，GitHub 不是产品必需，但 PR 路径显式依赖一个代码协作平台。

三条裁决。第一，显式引入并立为第五个模块 Repo（仓库），场景 Change（变更）：它按治理责任成立——仓库身份比任何 Project 活得久（独立生命周期）、变更集在执行体死后仍在（恢复边界）、合入钥匙只在控制面（权限边界），不按「有原生客户端」推。代价是一次分责调整：Repo 注册从 Project 挪出，Repo Instance 从系统层挪出，ChangeSet、ChangeSet Revision、Write Lease、集成意图与 Integration Receipt 从 Participant 挪出，`hctl2-tool` 成为该模块的现场执行者；Gate、Seat、Verdict 留在 Run，评审对象保留 Artifact Revision 分支，结晶归属按施工图先例分开（代码变更仍从 Terminal 会话结晶，对象归 Repo 模块）。第二，工具箱总在、平台按仓库绑定：纯本地仓库是完整的正常路径，代码协作平台是可有可无的 content 系统，GitHub 是缺省实现且本批只做它，其余后端不点名、按市场调研按需。第三，一处授权语义调整：平台一般只校验源头，给不了「预期目标头」的比较并交换（分支保护的「要求与目标同步」也不等于它），所以「合入 ChangeSet」命令的目标头处理改为两种授权形态之一——预期目标头形态默认，「接受目标前移」形态由人事前显式选择并冻结目标保护快照，凭证记实际目标头；放行不是执行时的降级。

同轮定下：发布评审的授权放在 Trigger Preview、范围写清、可选「须人显式确认」开关；评审评论经代取回到下一次调用；席位的一票是执行提交的提案而不是治理命令；操作模块后端按四级顺序（随包官方命令行 > 官方 SDK > 从接口描述生成 > 手写）。命名候选与理由、七项取舍的各家意见见[方案备忘](../../../.memo/design/scm-module-20260906/01-scm-module.md)；市场调研见[代码协作平台调研](../../research/scm-platforms.md)。

## 36. 只在本地的仓库改由随包的本地平台承接，选型 Gitea（v0.17.1）

2026-09-06 所有者拍板。§35 给「没有平台的仓库怎么办」的答案是一条不经平台的本地路径：讨论在 Scoped Room，检查是本地测试事实，合入由工具箱推进本地分支。所有者判定这不够：Repo 模块要在本地也完整可用，评审请求、评审线程、保护规则、合入记录这一整套过程必须有承载者。裁决是随包带一个本地代码协作平台，选型 Gitea——小、单二进制、功能全——给只在本地的仓库用；Forgejo 记录在案备选。所有者 2026-09-07 看到二进制一百多 MiB，要求把 Gogs 加入候选、能选就选；核对后落选：Gogs 的接口没有评审请求、评审、提交状态与分支保护，承载不了 PR 过程，解压后 87 MiB 也只比 Gitea 小四分之一，与 Vikunja、Dagu 同一量级。同日定打包压缩：安装包整包 xz（`-9 -T0`），Gitea 锁定上游 xz 制品下载，不用 UPX；压缩只减下载体积，磁盘上的二进制不变。

同轮定下一条判据，避免把两种情形混为「本地仓库」：仓库分两种，看它的平台在哪，不看它在不在本地。从 GitHub、GitLab 这类外部平台克隆下来的仓库，平台就是它来的那个，本机的克隆只是参与者自己的工作副本（Repo Instance），评审与合入在来源平台上走，本地平台不参与、不建镜像；只在本地的仓库没有任何平台承载，注册时缺省绑定本地平台，控制面在上面建仓、推送、给本机检出加远端，此后走与外部平台完全相同的一条链。

三点后果。第一，「没有平台绑定的 Repo 是完整的正常路径」这句改判：不挂平台改为注册时的显式选择、受限路径（仓库太大不想再存一份之类的理由），保留它的全部机械规则，但不再是缺省，也不再等于「本地仓库」；绑定了平台的 Repo 不再对本地目标开放集成。第二，本地平台是 control 托管的又一个 content 系统，与聊天服务、任务后端同列，随包不等于总在：能力逐项按实测声明，它不能保证预期目标头，本批不随包带流水线执行器，它上面的检查只有外部写回一种来源；停机时依赖平台回读的命令照样拒绝。第三，B2 的两条验收路径改为「缺省绑定本地平台的本地仓库」与「受保护的 GitHub `main`」。

未决：本地平台的存在给 Repo 身份提供了一个候选锚点（每个绑定了平台的 Repo 都对应平台上的一个仓库），与 §35 遗留的「仓库稳定身份」讨论一并处理，本条不改身份规则；显式不挂平台的路径是否最终取消，等 B2 走通后按实际需求再判。调研与调用面核对见[本地代码协作平台调研](../../research/gitea.md)，市场调研的复核记录见[代码协作平台调研](../../research/scm-platforms.md)。

<a id="37-单元模型"></a>
## 37. 单元模型：多个独立的控制面复用 Agency 与内容服务（v0.18.0）

2026-09-13 所有者拍板（方案 `.memo/design/case-study-20260907/10-batch-a-plan.md` v4，PR #222）。起因是所有者 2026-09-06 的多机用例（PR #191）：一个人、三台机器、两个控制面、三个 Agency、两个仓库、一个瘦客户端；#193 两轮评审从用例反推出十七条设计问题，第一条就是「没有单元模型」——三面架构讲职责分层，没讲什么可以独立装、独立活、互相怎么连。所有者 09-07 / 09-08 定了方向，原话是「按模块解耦」：Agency 独立于控制面；一个仓库同时被多个控制面开 Project 是常态；聊天归 Project；参与者自己推、控制面下令；前端不越过 HCTL 准入；模型凭据在 Agency；`hctl2-tool` 是参与者所在机器上的一组工具，不是单元。

转折：从已分责、但仍残留单一控制面和同机生命周期假设的架构，转为显式的多单元、多对多连接；三面分责保留。本批在架构正文立「单元与连接」一节——四类单元（控制面、Agency、前端、内容系统）加仓库底座，判据两条与义务两条，部署对等不是权威对等，放置 / 权威 / 交付三种关系，单元之间的总规则；改口「一人多机只连同一控制面」「随 HCTL 一键启停」「运行时失联即暂停」和几处「部署由三面回答」；愿景补两句白话与设计原则第 17 条；根首页与设计地图加入口；交付范围改两期。本批不改约束句。

已由 #210 / #211 与 09-11 / 09-12 裁决落地、不再列为待办：工种归 Agency、两次选入取代雇佣、参与者授权与 Participant–Agency Binding 撤销、并发留给参与者、不做会话分叉。未完成的范围按后续批次登记，之后每批在台账加一行指回本节：

| 批 | 未完成范围 |
| --- | --- |
| C Repo 与治理正文（v0.18.1 已落约束） | Repo Instance 与通用现场代次退出；登记用人的记录及声明的平台绑定，工作副本由参与者管理、引用事前不相交；共同平台多写按各自冻结契约核验；推送由持凭据单元执行，一条发布意图分两阶段；审计关联；治理记录与正文两半、存取/准入/备份及 Context 交付；#216 F4 的 C 范围与「治理正文」词条已处理 |
| B 系统边界与 Participant（v0.18.2 已落约束） | Agency 唯一通路与租户隔离（原「非端点独占」按裁决改为多租户）、信任边界与本机代管、`hctl2-tool` 部署位置与报告通道（直报三路）；「一人多机连同一控制面存储」改口；工种接受条件、待命归 Agency 并落交付文档、证据三档及 R2 撤销、联系不上 / 截止已过 / 身份不可证三分、权限分层、派工四步与恢复七步；Execution Runtime（改名派工）、`runtime_generation`、Agency binding owner generation 退休 |
| D 看板、Run 交接与交付（v0.18.3 已落约束） | 任务源零到多个、缺省源由人显式选定（缺省建议平台自带 issues）、一张卡一个家与认领不搬家、两套分组与脱钩、合并板投影、多写通则的任务后端实例、源停用后保留 Task；施工图 lint 的票数检查（能证明的拒绝加重复风险提示）；跨机返工沿冻结选入记录、重建只用封存版本；正式参考用例 `scenarios/S1-multi-unit.md`；交付顺序改平台 issues 先；P2 计划补记 |

C 批按所有者 2026-09-14 的 [#224 拍板](https://github.com/yesme/hctl2/pull/224#issuecomment-5654759113)及[原话补记](https://github.com/yesme/hctl2/pull/224#issuecomment-5655197128)落约束：本控制面不推断、去重或合并仓库身份，§36 未决段里的「仓库稳定身份」据此收口，跨机与平台迁移不另成身份题；§36 的「控制面推送」改为持凭据单元执行，初始建仓交代码亦然，本地平台缺省不变。§36 保留为历史原文。工作副本、材料存放、治理准入与交付分责以现行约束为准，不从代码树或平台结果倒推本控制面授权。

B 批按所有者 2026-09-14 的 [#228 裁决](https://github.com/yesme/hctl2/pull/228#issuecomment-5663885388)与「拍板 · B 批方案」落约束：Agency 是控制面与参与者之间唯一的通路且功能不泄露——像一台裸金属上给每个配对的控制面开一个隔离租户，门后的进程、会话、主机、待命与换人是 Agency 的内部；控制面只认派工（原 Execution Runtime 改名）、自己签发的授权对象与归到派工的结果，物理代次与端点归属者代次删除，代次家族从五种减到三种；参与者卡住由 Agency 在冻结规格内自愈，规格外报无法履约，Agency 不可达只是联系不上，换 Agency 由人；待命是 Agency 的实现细节，所有者的待命原则作为产品要求写在交付文档；Gate 多样性只留 Worker Profile 互异与回避作者（#216 R2 由所有者撤销：「没法太严格」）；证据三档直报 / 旁路 / 转述，参与者内部那一路的直报按 Agency 是否声明「代为执行工具并直报」记。所有者对本批方案第一稿的批评（「I'm concerned and questioning 你们的 architecture competence」）指向同一件事：控制面不需要知道门后换了人，多租户不是逐次核代次。

D 批按所有者 2026-09-15 的 [#230 拍板](https://github.com/yesme/hctl2/pull/230#issuecomment-5668292895)落约束：看板多源按所有者 2026-09-06 原话——一边 GitHub 一边 Linear 并不罕见，缺省任务源列选项让用户选、缺省给平台自带的 issues、必须显式同意；一张卡一个家、换卡不做（「卡不搬家——如果卡后端挂了，那就是 control/workbench 的后端挂了，那还搬啥搬？ground-truth 都在那里」）；施工图 lint 的票数检查按所有者 2026-09-13 原话落，检出后只提示不拦（拍板乙），能证明凑不足的才拒；任务后端交付顺序改平台 issues 先（拍板甲）；跨机返工按用例必然情形 7；一个动手 PR。改写 DAG 至此四批走完。

改写的规矩与施工图见 `.memo/design/case-study-20260907/05-rewrite-process.md` 与 `08-rewrite-dag.md`。
...
本批同步直接受影响的约束、设计引用和验收要求；独立授权、无 Run 路径、Run 完成与 Task 验收的分离不变。产品实现与行为实测另行记录，不因文档检查通过视为交付。
...
| 版本 | 日期 | 内容 | 详情 |
| --- | --- | --- | --- |
| v0.10.3 | 2026-08-19 | 词汇清扫：RuntimeBackend、TaskSource、WorkflowEngine、HarnessAdapter 四个未入册高频名降级为描述语或端口种类；交付文档定为与约束层同侧 | [清扫表](#v0103-清扫) |
...
| v0.16.5 | 2026-09-04 | 四轴评审第三轮收口批（承接 Grok 对 #156 的修正结论）：首现中文对照检查器的 46 条豁免全部写回正文——十个有自然译名的核心产品词与三个高频约束词在九份设计层文档首次出现处补对照，四份模块正文的标题随之带对照，横切正文标题改为「可解释的 Context（上下文）」；词汇表与架构正文的 Execution Spec 对照由「派发规格」对齐为约束层总则的「执行规格」；豁免表清零并由夹具锁住只能保持空表；不改约束语义 | [任务书](../../../.memo/design/design-review-20260902/30-r3-remaining-brief.md) |
...
| v0.17.3 | 2026-09-11 | 三组研究落设计正文（治理功能消亡 33 家、多 agent 协作有效性、Prove2Me 依赖图）：愿景补「中段判断门无人守住」的行业证据与「省」的标尺、自举是证明门在主路径上工作的办法；Run 正文重写评审独立性（多样性三层、人设不算、同配置同包只算一票、分歧不设商量回合）并新增「施工图怎么凝结」（节点判据二选一、边由机械物承载、背景写成「上下文」章节不分叉会话、读回默认开且默认换执行配置、运行中不归约子节点）；Participant 正文收窄专业化说法（Skill 改上下文、人设是界面属性）；Context、Project、Task 正文各补一句；交付默认值返工上限改 2 轮、自举加门触发计数与逃生口验收；Run 正文写施工图归约的七条抑制规则与关键路径度量；Participant 模型重排——工种（Agency 名册）→ 选进 Room 的规划者 / 选进 Run 席位的施工者 → 执行体，撤销「参与者授权」与 Participant–Agency Binding，系统角色名 worker 改执行体；不改约束语义 | [裁决记录](../../../.memo/design/design-review-20260902/21-r2-rulings.md) |
| v0.17.4 | 2026-09-11 | 三组研究与 Participant 模型重排落约束层：选入记录只在连接约束定义一次，Room 名册与 Run 席位各持一份，Execution Spec 冻结选入记录引用、Project 选人策略摘要与本次实际选用的 Worker Profile；两次选人各自独立、两处准入校验选人策略，活动 Run 不换施工者，候选切换只在获准候选范围内换 Worker Profile 与执行体；Gate 策略加席位多样性字段，计票去重（投票时实际 Worker Profile 摘要与交付内容摘要均相同只计一票）总是生效，重复组内裁决不一致按最不利处理，去重后票不够按法定票数不可达，席位包不含他席裁决与控制面附加的作者标识，分歧不设商量回合；Profile 规则四——达成判据二选一且与开工前置分开、产物依赖的边引用上游声明的产出、静态图无环且落地、批准后节点集不变；「批准 Workflow」前置读回记录（有类型的票据、四条机器判准入、可声明关闭、可声明换 Worker Profile）；Workflow Revision 可带「上下文」章节，不产生节点与判据；Gate Receipt 逐票记选入记录、实际 Worker Profile 摘要、交付内容摘要、去重标记、逐条证据等级与 token 用量；撤销参与者授权与 Participant–Agency Binding；CT-RUN 与 CT-PRODUCT 相应改写 | [裁决记录](../../../.memo/design/design-review-20260902/21-r2-rulings.md) |
| v0.17.5 | 2026-09-12 | 词汇改口：架构与约束层特指 `hctl2-tool` 这款命令行工具时写组件名，全库「工具箱」改口（决策史正文不换词），证据通道 `toolbox_readback` 的语义名同改，词汇表留旧称对照，「工具箱」进退休词表；不改约束语义 | [裁决记录](../../../.memo/design/design-review-20260902/21-r2-rulings.md) |
| v0.17.6 | 2026-09-12 | 合并后一致性修正（#216 F2、F3、F5 与两处计数）：四处把控制面存储写成行为主体的句子改回——权限与判定由控制面执行、选人由控制面准入并记录、只追加的是治理事件记录，事项清单按 09-11 不动；凝结「六条」改七条、Profile 规则「三组」改四组；约束层写明控制依赖只指结构件所连的边、两个外部执行节点之间不带引用的边按产物依赖落地失败拒绝并配 CT；验证器 S1.I13 与参与者对照表三行按 09-11/09-12 裁决更新，作者推论标明 | [#216 报告](../../../.memo/review/20260912-v0.17.5/codex-20260912.md) · [施工图](../../../.memo/design/case-study-20260907/08-rewrite-dag.md) |
| v0.17.7 | 2026-09-13 | Run 计票与读回口径（R 批，所有者逐条拍板）：约束层新增「计票与结束」——有效裁决、席位仍可产出、席位仍可能加入、重复组已定四个可算的定义与五步求值顺序，重复组等到没有席位还可能加入时才算一票、上界以全部有效裁决为固定输入、上界少于法定票数才确定不可达、否决席未投不得签通过也不得被撤、返工上限不进不可达定义、恢复后不等收齐；启动前已能证明凑不够时启动拒绝；三张清单随 Workflow Revision 登记为附属正文，批准核登记记录、读回记录与实际交付 Bundle 三处摘要一致；登记记录含产出来源（调用集合 / 显式无 / 缺失），回避声明按集合求值、缺失不判满足；设计层与交付缺省同步，CT-RUN 配对应用例 | [R 批方案](../../../.memo/design/case-study-20260907/09-batch-r-plan.md) · [施工图](../../../.memo/design/case-study-20260907/08-rewrite-dag.md) |
| v0.18.0 | 2026-09-13 | 架构批：单元模型进架构——「单元与连接」一节（四类单元加底座、判据与义务、部署对等不是权威对等、三种关系、总规则）；三面保留讲职责，文首与几处「部署由三面回答」改口；「一人多机只连同一控制面」「随 HCTL 一键启停」「运行时失联即暂停」三句改到架构层原则；愿景两句白话与设计原则第 17 条；根首页与设计地图加入口；交付范围改两期；词汇表加单元、底座、三种关系；不改约束句 | [§37](#37-单元模型) · [A 批方案](../../../.memo/design/case-study-20260907/10-batch-a-plan.md) |
| v0.18.1 | 2026-09-14 | C 批：仓库登记与工作副本分责；共同平台事实按事先采纳的契约经 Repo 核验，批准与完成仍各控制面判；源版本由持凭据单元交付（含初始建仓），发布评审分段确认与按范围公开审计关联；控制面存储分治理记录与治理正文，材料保存/准入/交付、保管与一致备份、可重读 Context 副本同步；§36 身份疑问在§37 收口，历史原文不改；契约测试沿既有族更新，不改 B/D 后续范围 | [§37](#37-单元模型) · [C 批方案](../../../.memo/design/case-study-20260907/11-batch-c-plan.md) |
| v0.18.2 | 2026-09-14 | B 批：Agency 是唯一通路与多租户——控制面对象 Execution Runtime 改名派工、删物理代次与 Agency 端点归属者代次（代次三种）、派工四步 / 总表 / 失败表 / 恢复七步 / 备份一份合同；信任边界改缺省回环加非本地认证，本机代管是一种部署；证据三档（`toolbox_readback` 改 `unmediated`，`hctl2-tool` 输出的 `evidence_level` 同改）与「代为执行工具并直报」能力；工种接受条件；待命与 Herdr 退到交付文档；联系不上 / 截止已过 / 身份不可证三分；Gate 多样性只留两项；架构加「后端不响应」总则与 Agency 阶梯；CT 沿既有族更新；不改 D 后续范围 | [§37](#37-单元模型) · [B 批方案](../../../.memo/design/case-study-20260907/12-batch-b-plan.md) |
| v0.18.3 | 2026-09-15 | D 批：任务源零到多个、缺省源由人显式选定（缺省建议平台自带的 issues）、一张卡一个家与认领不搬家、两套分组与脱钩（三句旧限制换掉）、合并板投影、多写通则的任务后端实例、源停用后保留 Task、换卡不做；施工图 lint 票数检查（能证明的拒绝加重复风险提示，同一条去重键）；跨机返工沿冻结选入记录、重建只用封存并获准交付的版本、无 Run 由人选人；参考用例 S1 落 `scenarios/`；交付顺序改平台 issues 先、本地任务服务器可加绑；P2 计划补记；CT 沿既有族更新。改写 DAG 四批收口 | [§37](#37-单元模型) · [D 批方案](../../../.memo/design/case-study-20260907/13-batch-d-plan.md) |
| v0.18.4 | 2026-09-17 | 对照收口：愿景写明长期主体是项目与承诺、协调靠制度不靠对话（两条轴），「不解决什么」加不以数字伙伴为组织中心，原则 2 加「没有新信息的协调不进模型循环」与省要按总账实测，延长线改为伙伴推荐与外部数字员工两条；设计地图加五模块制度表；Participant 正文加「从选人到被推荐」延长线（召回、可行性过滤、排序、任用，履约记录的键，不另建 Task/Run）；context 与 run 两句收窄；轻量路径预填下一次预览；交付加缺省施工图模板与「省」的验收总账；参考用例 S2 四种不顺利情形；研究文件两条复核记录。不改约束语义 | [主笔处置](../../../.memo/notes/grokbot-compare/claude-disposition-20260917.md) |
| v0.18.5 | 2026-09-17 | 交付文档未决问题裁决（所有者 2026-09-17 逐条）：关闭两条旧未决——「ChangeSet 默认基数」的前提「多 Task Run」自 v0.7 已不存在，Task 与 Run 一对多、Run 与评审请求一对一写进 spec/repo §变更与平台的映射；「原生会话导入」非所有者需求，设计正文整节删。Task 依赖归任务源、只投影，只有阻塞的派工由人确认；Scoped Room 闲置提示与预算耗尽的缺省落定；安全策略原则进 CONSTRAINTS，隐私与远程两条随安全批 | [#243](https://github.com/yesme/hctl2/pull/243) |
| v0.18.6 | 2026-09-17 | 安全批（所有者 2026-09-17「先默认安全环境，安全是可配置的 policy，不要在每个小地方各解决一次」）：spec/system §安全边界 改为「安全策略面」，成为全库安全文本唯一落点——七个策略点各写管什么、当前缺省、不可配置的底线（端点与连接、租户隔离、凭据、执行加固、房间隐私与保留、客户端最小权限、敏感数据），当前生效取值可经公共查询面读到；租户隔离与端点认证的定义并入，密钥存放、通用凭据禁交、跨租户不可达、安全输入去处各处改指针；桌面壳具体开关移到交付文档技术基线；「远程认证仍属未决」与「Repo Room 隐私未决」两条改为策略点的当前缺省与后续取值；CT-SYSTEM 加策略缺省用例 | [#244](https://github.com/yesme/hctl2/pull/244) |
| v0.18.7 | 2026-09-19 | 体验澄清与场景验收同步：Project 主 Room、Topic 开场材料与关闭、多 Source 入口、Task 映射按 Project 独立、待处理投影与保存计划；保留同 Repo 多 Project，S1 纠错、S2 对照界面、S3 与既有 CT 族补失败用例 | [§38](#38-体验澄清project-各自主-roomtopic-与多源入口v0187) · [#257](https://github.com/yesme/hctl2/pull/257) |
| v0.18.8 | 2026-09-19 | G 批第 2 批（全库对照 v0.18.7 核对，施工图来源族与「哪份 Git」族）：施工图可由 Room 讨论凝结或从 Task 模板填表而来，版本、批准与 Task 关联归 Run；结晶归属按实际来源判断；判决权威在治理记录、审计副本在治理材料、公开范围不随仓库隐私推定；读回规则在确无来源 Room 时只有该 Room 名册回避项不适用，其余隔离与可核条件保留，配正反例（所有者 2026-09-19 裁 A5）；不改其他约束 | [G 批方案 §八](../../../.memo/design/case-study-20260907/16-batch-g-plan.md) · [#264](https://github.com/yesme/hctl2/pull/264) |
| v0.18.9 | 2026-09-19 | G 批第 3a 批（全库对照 v0.18.7 核对，语义范围族与单元）：承诺、授权与验收以 Project 为范围，Repo 级代码与平台事实仍按 Repo 管理（所有者 2026-09-19 裁 A1）；Project 不是部署单元，共享资源不合并归属与授权；引用保留对象既有作用域；备份剔除 Agency 的 Skill 安装定义，保留治理记录、治理正文与承诺保存的字节；Attempt 切换只在 Agency 报无法履约后；文档纪律加体验目录一条；#263 三条原则的三处落点（愿景入口原则、组织层可观测、承诺成本入总账）；不改其他约束 | [G 批方案 §八](../../../.memo/design/case-study-20260907/16-batch-g-plan.md) · [#263 备忘](../../../.memo/design/intent-entry-principles-20260919.md) · #265 |
| v0.18.10 | 2026-09-19 | G 批第 3b 批（全库对照 v0.18.7 核对，Task 归属与开关缺省）：「发布评审须人显式确认」开关的缺省归 Project 版本化设置、随本次授权冻结（所有者 2026-09-19 裁 A2）；Run 绑 Task 与 Room Invocation 采纳契约都核同 Project；归档拒绝清单、Task 归属不可变句、「注册不另建仓库级 Room」各收成一处权威；接入任务源与汇总投影的用词统一；CT 分组行按获准映射改写；不改其他约束 | [G 批方案 §八](../../../.memo/design/case-study-20260907/16-batch-g-plan.md) · #267 |
| v0.18.11 | 2026-09-19 | G 批第 4 批（全库对照 v0.18.7 核对，Overview 与归档恢复）：Project Overview 保留为只读投影、不要求独立入口、不替代主 Room 与「待你处理」（所有者 2026-09-19 裁 A3）；「需要关注」是对象标记，不另成待办；Topic Room 命令词「创建 / 关闭」，关闭即已归档；Project 恢复后随归档转只读的开放对象恢复接收命令、已终态不复活、已关闭 Topic 不随恢复；引用 Run 不改其授权、不承担交付，配 CT；不改其他约束 | [G 批方案 §八](../../../.memo/design/case-study-20260907/16-batch-g-plan.md) · [#268](https://github.com/yesme/hctl2/pull/268) |

<a id="36-当前设计"></a>
## 当前设计

这条来时路最终收敛为五个权威模块、五个对仗场景、共享但受控的连接与执行机制。阅读当前设计时，应从[愿景](../vision.md)开始，再读 [Project](../project.md)、[Task](../task.md)、[Run](../run.md)、[Participant](../participant.md)、[Repo](../repo.md)，再查看[连接约束](../spec/connections.md)、[系统边界](../spec/system.md)和[交付文档](../delivery.md)。本文用于解释这些边界为什么存在；发生冲突时，它不覆盖任何当前规范。
