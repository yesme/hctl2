...
| v0.18.4 | 2026-09-17 | 交付文档未决问题裁决（所有者 2026-09-17 逐条）：关闭两条旧未决——「ChangeSet 默认基数」的前提「多 Task Run」自 v0.7 已不存在，Task 与 Run 一对多、Run 与评审请求一对一写进 spec/repo §变更与平台的映射；「原生会话导入」非所有者需求，设计正文整节删。Task 依赖归任务源、只投影，只有阻塞的派工由人确认；Scoped Room 闲置提示与预算耗尽的缺省落定；安全策略原则进 CONSTRAINTS，隐私与远程两条随安全批 | [#243](https://github.com/yesme/hctl2/pull/243) |
