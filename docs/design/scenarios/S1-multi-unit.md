...
| S1.X2 | 两个控制面各在 GitHub Issues 上有 Task；同一张 issue 被两边各认领一张 | [01 §五「看板：多源（v3）」](../../../.memo/design/case-study-20260907/01-unit-model.md#五看板多源v3)推论；现行处置见 [Task 契约与来源](../spec/task.md#契约与来源)、[Task 写入约束](../spec/task.md#写入约束)；所有者把「两控制面同改一张卡」归多写通则（原 S1.I12） | 各控制面分别按源投影，可选汇总不合并治理身份；卡内容以 GitHub 为准，治理状态各自独立；写回评论带控制面与 Task 标识；人的原生 Done 按各自绑定成请求 | CT-TASK「另一 Project 或另一控制面的自动写回触发本方 Task 或 Run 完成时失败」「可选合并板被当作权威表写入时失败」 |
...
| S1.X4 | Linear 绑定被停用 | [01 §五「看板：多源（v3）」](../../../.memo/design/case-study-20260907/01-unit-model.md#五看板多源v3)（Kimi）；现行处置见 [Task 契约与来源](../spec/task.md#契约与来源)、[Task 写入约束](../spec/task.md#写入约束) | Task 保留、标需要关注；重新接通同一实体即恢复；换卡不做 | CT-TASK「家所在的源停用后 Task 与认领保留……」 |
