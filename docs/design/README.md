...
第一阶段采用 Tuwunel、Vikunja、Dagu、Herdr，并不把四个模块绑定到它们的私有对象模型。替换边界是各模块自己的受控端口和版本化 binding，不是一套跨模块的通用 shim 服务；Workbench 只依赖 HCTL 场景合同，provider 原生界面只作可选 content 客户端或诊断工具。四类边界、替换范围和聊天桥接的职责划分见[三面架构的“避免供应商锁定”](./architecture.md#避免供应商锁定)。
...
- 来时路只记录转向：核心边界移动、实现选型更换、权威归属变化各自成章；词汇、词形与概念清扫类修订进[小修订台账](./references/decision-history.md#30-小修订台账)一行，细节在合同层清扫表或 memo。每记录一条改变合同的转向，基线版本至少推进一个补丁号。
