...
本节只管**内容与权威**——谁定义什么、去重、引入门槛。**文风与结构**（分层语域、句式、术语混排、翻译腔、压缩下限）见[写作指南](../../WRITING-GUIDE.md)。为避免再次形成补丁链，后续修改遵守以下硬边界：

- 分层写作：设计正文只用[核心产品词](./spec/README.md#核心产品词)加日常语言；约束词汇（复合对象名、状态机、字段）只出现在约束层；实现名（字段名、锁路径）不进设计文档。`delivery.md` 是验证文档，可引用约束层词汇以指认被测约束，但不得重定义。
...
- 来时路只记录转向：核心边界移动、实现选型更换、权威归属变化各自成章；词汇、词形与概念清扫类修订进[小修订台账](./references/decision-history.md#34-小修订台账)一行，细节在来时路核销表或 memo。每记录一条改变约束的转向，基线版本至少推进一个补丁号。
...
发生冲突时，四个模块约束解释连接端点，spec/connections.md 解释交接，spec/system.md 解释共享执行机制；delivery.md 不得改变领域含义，实现证据不得反向定义产品。
