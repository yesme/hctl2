...
3. 没有影响必需输出的未决副作用；
4. Manifest、Engine Execution Binding 和结果引用仍匹配。
...
Obligation 的身份、截止时间与租约都是账本事实；引擎侧的 DAG run ID 与不可变步骤名只作 Engine Execution Binding 的关联键，代次不在引擎。超时与备用候选准入只依据账本自己的 Obligation 截止时间。引擎重试替代旧 Obligation；control 先使旧 Obligation 及其未终态 Seat/Attempt 的派发、写入与输入授权失效并提交物理隔离 outbox，再按新的观察序号创建新 Obligation。候选切换只替代同一 Seat 下的 Attempt，不增加票数或更换逻辑裁判。
...
完成谓词只依据账本事实与外部证据的当前回读。引擎停报进度时，control 只把 Engine Execution Binding 标为分歧，不阻止 Run 完成。
