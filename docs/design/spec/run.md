...
| Run | 固定归属一个 Project，对冻结 Workflow、可选 Task、基线、角色、候选、权限和预算的一次授权执行；从哪个 Room 或 Task 入口查看不改变归属 |
...
| --- | --- | --- | --- |
| Workflow Revision / Engine Deployment | immutable revision + approval version | control 协调「登记/编译/批准」命令；登记可携带同一 Project 内的 Task 引用，关联随登记一并准入并由本模块保存；登记携带三张清单的快照与来源链（关闭读回时不强制）及产出来源（调用集合、显式无、缺失）；正文与清单由控制面保存并核验；固定 compiler/adapter 产出及摘要 | Revision 不改写；新内容创建新 Revision |
...
任何一项未知都不得完成 Run。Engine 检查点、进程退出、Harness 或模型自述和单个 Result Proposal 都不能补足上述谓词。引擎报告的进度只用于分歧检测；该进度不可读或与治理记录不一致时，control 只把 Run–Engine Binding 标为分歧待对账，既不补足也不否定上述谓词。
...
Obligation 的身份、截止时间与租约都是治理记录；引擎侧的 DAG run ID 与不可变步骤名只作 Run–Engine Binding 的关联键，代次不在引擎。超时与备用候选准入只依据控制面记录的 Obligation 截止时间。引擎重试替代旧 Obligation；control 先使旧 Obligation 及其未终态 Seat/Attempt 的派发、写入与输入授权失效并提交物理隔离 outbox，再按新的观察序号创建新 Obligation。候选切换只替代同一 Seat 下的 Attempt，不增加票数或更换逻辑裁判。

Verdict、Gate Receipt 与凭证链是 Workflow 场景的结晶（“干成了的证明”）：权威在治理记录，审计副本在治理材料，公开范围不随代码仓库隐私推定；边界见[系统存储约束](./system.md#git-的双重角色)。施工图（Workflow Revision）由 Room 讨论形成时，其讨论结晶归 Room 场景；模板等其他来源不要求先有 Room 讨论；版本、批准与 Task 关联归本模块，按实际来源判断，不按登记形式推断。
...
“启动 Run”必须以比较并交换校验活跃 Project/version、可选 Task 的开放生命周期/current Revision，以及该 Task 的 Run 占用标记。在同一用户级控制面事务中，control 创建 Run、不可变 Manifest、幂等结果、`active` Task 占用标记和引擎启动 outbox。
...
运行中只有 Manifest 明确声明为可变的放置参数可以按冻结规则和边界调整；放置参数指控制面授权范围与对外交付目标，Agency 内部搬机、换目录不是放置调整，不更新 Run version、不产生审计。每次调整都校验预期 Run version，并留下固定前后值、适用规则、actor 和 Run version 的不可变审计事件。范围、验收、候选、权限、Gate 或超出获准边界的放置变化必须创建替代 Run，不能原地漂移。获准的追加预算是唯一例外：经 Request 记录人的追加授权，作为后续派工与尝试的授权依据，不回写历史 Manifest（见[Request、重试与 Gate](#request重试与-gate)）。
...
预算是 Manifest 冻结的授权的一部分，硬在「不再开新的」：预算耗尽后 control 不再创建新的派工或 Attempt，也不为候选切换再开尝试；正在跑的派工按冻结策略处理，缺省跑完并照常准入，施工图或调用可声明「超限即取消」；追加预算走 Request，获准的追加记在该 Request 上作为后续派工与尝试的授权依据，不回写历史 Manifest，获准后才能再派。用量以 Agency 上报为准，报不全的标未知并按估算计入，不当作零。
...
四个词都由已记录事实与冻结数据算出。**有效裁决**：席位已经过结果准入的裁决；已冻结、尚未准入的提案既不计票，也不当作没有。席位**仍可产出**：义务仍有效、未到截止，且有尚未准入的提案、有尝试在跑、或按冻结策略发生获准的技术故障后候选、预算与剩余截止仍允许再开尝试——尝试失败、丢失、被撤权都归到能否再开尝试，能则仍可产出，不能则候选耗尽。席位**已结束**：有了有效裁决，或到截止、候选耗尽、被取消、被替代；由既有准入与撤权规则判，不由 Attempt 终态单独决定，「已交提案」是 Attempt 终态而席位不算结束。席位**仍可能加入**某个重复组：它仍可产出，且它可能产出的有效裁决的键能与该组相同——Worker Profile 摘要按选入记录的获准候选范围判；交付内容摘要在 Bundle 已交付时用实际交付摘要，尚未交付时按已冻结的引用（ReviewSubjectRef、席位必需 Skill 的引用摘要、根 Manifest）判能否证明不同，证明不了不同即算可能相同；已结束的席位不再可能加入任何组。重复组**已定**：组内至少一张有效裁决，且没有席位仍可能加入；组的裁决取组内有效裁决里最不利的一个，否决优先；没有任何有效裁决的组不形成票。

每有新事实，control 按五步重算，只用已记录事实，不等待"收齐"：一、按本节校验并准入席位结果；二、按有效裁决的键分组，对每组算仍可能加入的席位；三、先按冻结规则处理否决、技术失败与非通过裁决——策略声明的否决票按本节触发返工路径，`rejected` / `changes_requested` 进归约器、契约内分歧标需要关注，技术故障按候选切换或类型化技术失败与 Request；四、已定的通过组数达到法定票数、没有仍可产出而无有效裁决的否决席、且剩余席位按冻结规则不能否定通过时，签汇总 Verdict/Receipt 并撤销剩余 Attempt，被撤销席位的组就此不形成，其后交回的提案只留历史；五、否则计算上界——以当前全部有效裁决（含未定组里的）为固定输入，对仍可产出的席位取对通过最有利的合法落点，两部分按实际键去重、按冻结规则归约，得到最多可能取得的通过票数——上界少于法定票数即**确定不可达**，返回法定票数不可达；否则**暂未收齐**，继续等待，等待由各席位的已结束（截止、候选耗尽、取消、替代）保证有界。

策略声明了否决权的席位没有有效裁决而仍可产出时，不得签通过凭证，也不得为签通过凭证把它当作剩余撤销；其尝试失败时仅在义务仍有效、发生获准技术故障且候选、预算、截止允许时切换，已交提案不切换，已取消、被替代不重启；否决席最终没有取得有效裁决时按候选耗尽处理——创建 Request 或标类型化技术失败，不签通过凭证。组籍在有效裁决产生时按实际键定，候选切换让两席中途成组或拆组都照键算，只在切换违反已声明的多样性时拒绝。组的形成是对当前 ReviewSubjectRef 的谓词，返工产生新 Revision 后旧票失效、新一轮从零起。恢复后沿本节与[连接约束](./connections.md)的准入、去重、截止与撤权规则收取结果并重算，不要求先证明全部供应端的已封存结果已经收齐。

按上述规则签出汇总 Verdict/Receipt 后，control 撤销剩余 Attempt，再完成 Engine 检查点。Gate Receipt 逐票记录选入记录引用与冻结版本、投票时实际使用的 Worker Profile 摘要、交付内容摘要、去重后是否计入法定票数、每条所引证据各自的通道等级，以及本票的 token 用量——已回读的实际用量与估算分别记，缺失标 unknown，不填零。未达门槛不签汇总凭证，各票留在裁决与审计记录。上述规则判定确定不可达时，Gate 返回法定票数不可达（`quorum-unreachable`），使 Obligation 失败并沿 Workflow Revision 的失败边推进。返工产生新 Revision 后，旧票失效，新的 Revision 必须重新通过 Gate。Gate 策略可声明**返工轮数上限**（默认值见[交付文档](../delivery.md#运行默认值)）：汇总要求继续返工而再开一轮将超过上限时，按法定票数不可达同路处理——Obligation 失败并沿失败边推进，或按策略创建 Request 找人；本轮已取得的合格结果不因轮数恰好用满而失败；不得无上限返工。Gate 策略还可声明**增量评审**：新 Revision 的评审包附带与上一版的差异指针，席位可只读差异；未声明时每轮全量重评。Task Revision 只有在验收契约变化时才更新。
...
随后 Task 按同一个用户级控制面存储中的当前 Revision、来源的新鲜度与分歧和逐项证据独立校验，并在成功 Receipt 或持久拒绝结果的事务中清除该占用标记。Task 拒绝不回滚 Run。失败、已取消或被替代的 Run 只有满足上一段隔离前置，其终态事务才释放旧占用标记；它们只形成需要关注或历史，不提交完成或取消 Task。

完成谓词只依据治理记录与外部证据的当前回读。引擎停报进度时，control 只把 Run–Engine Binding 标为分歧，不阻止 Run 完成。
...
| 引擎外部检查点 | Dagu 的 processless `human.task` / Camunda 的 external task 模式 | Dagu 名称虽含 human，在 HCTL 中只是引擎报告的进度点：control 观察其等待态后创建 Obligation，控制面记录的结果落定后再推进检查点；场景客户端不得直接操作 |
