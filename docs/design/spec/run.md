...
预算是 Manifest 冻结的授权的一部分，硬在「不再开新的」：预算耗尽后 control 不再创建新的派工或 Attempt，也不为候选切换再开尝试；正在跑的派工按冻结策略处理，缺省跑完并照常准入，施工图或调用可声明「超限即取消」；追加预算走 Request，获准后才能再派。用量以 Agency 上报为准，报不全的标未知并按估算计入，不当作零。
