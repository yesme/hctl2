...
每有新事实，control 按五步重算，只用已记录事实，不等待"收齐"：一、按本节校验并准入席位结果；二、按有效裁决的键分组，对每组算仍可能加入的席位；三、先按冻结规则处理否决、技术失败与非通过裁决——策略声明的否决票按本节触发返工路径，`rejected` / `changes_requested` 进归约器、契约内分歧标需要关注，技术故障按候选切换或类型化技术失败与 Request；四、已定的通过组数达到法定票数、没有仍可产出而无有效裁决的否决席、且剩余席位按冻结规则不能否定通过时，签汇总 Verdict/Receipt 并撤销剩余 Attempt，被撤销席位的组就此不形成，其后交回的提案只留历史；五、否则计算上界——以当前全部有效裁决（含未定组里的）为固定输入，对仍可产出的席位取对通过最有利的合法落点，两部分按实际键去重、按冻结规则归约，得到最多可能取得的通过票数——上界少于法定票数即**确定不可达**，返回法定票数不可达；否则**暂未收齐**，继续等待，等待由各席位的截止、候选耗尽与终态保证有界。
