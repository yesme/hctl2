...
| --- | --- | --- | --- |
| Repo | stable `repo_id` + `repo_version`；待确认 / 活跃 | control 处理「注册 Repo」命令；工具箱写入/回读 Git identity 并为本机检出登记远端；缺省绑定本地平台时，平台适配器执行「在本地平台建仓并推送」这条外部副作用命令，它有自己的幂等键与冲突范围 | 一个 Repo identity 只有一个 Repo 与 Repo Room；待确认的外部写入按关联键恢复，不重复注册、不重复建仓 |
...
**本地目标正被检出时的正常入口。** 只对显式不挂平台的 Repo：目标 ref 正被某个 Git 工作树检出时，工具箱默认拒绝，并返回精确的工作树路径与恢复动作；正式走法是人先把该工作树切离目标 ref，再重试同一意图。显式放行只推进 ref、不更新该工作树的文件与索引，是运维例外，不是正常路径。
...
本地平台的绑定与外部平台一样逐项声明能力，按调研的实测结果填，不因为随包就默认全有。本批不随包带流水线执行器：本地平台绑定的「检查」能力声明为「无独立检查，仅外部状态写回」——契约要求独立的平台检查时，这个绑定按缺能力处理（等待、拒绝或标为需要关注）；control 按工具箱回读的本地测试事实写回提交状态只是记录，不单独构成证据；本地平台不能保证预期目标头，与已核对的外部平台相同。本地平台上的账号由 control 创建：一个管理员账号给 control，适配器持其令牌；有权的人各一个普通账号；账号映射照常写进绑定。「不为此新建账号系统」指的是不另建平台之外的账号体系。
