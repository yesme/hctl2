...
注册命令必须定下平台绑定，三选一：绑定仓库来源的外部平台；绑定随包的本地平台；显式不挂平台。判据是仓库的平台在哪，不是仓库在不在本地。从外部平台克隆的仓库绑定该平台，本机的克隆只是 Repo Instance，本地平台不为它建镜像，把它绑到本地平台的请求拒绝。只在本地的仓库缺省绑定本地平台：control 持久化注册意图后，由平台适配器在本地平台创建对应仓库并推送现有分支，工具箱为本机检出登记指向本地平台的远端；之后该检出是 Repo Instance，评审请求、检查、保护条件与合入都在本地平台上走，与外部平台共用同一平台端口、能力声明、授权形态与恢复规则。在本地平台建仓与推送是外部副作用命令：结果未知时按关联键回读，不重复建仓；本地平台不可用时注册保持待确认，不改走不挂平台的路径。
...
**目标按种类固定。** 目标是平台端口所绑定平台（外部平台或本地平台）上的远端 ref，或某个 Repo Instance 上的本地 ref；本地 ref 只对显式不挂平台的 Repo 开放，绑定了平台的 Repo 提交面向本地目标的意图时拒绝。意图固定目标种类、ref 名与所用绑定版本。面向本地目标的意图由工具箱以比较并交换推进 ref；面向远端目标的意图由平台适配器请求平台合并。同名的本地分支与远端分支是两个目标，对本地 ref 的集成不得表现为远端 ref 已合入。
...
显式不挂平台的 Repo 走受限路径：契约在采纳时不得要求平台上才有的机械证据；机械验收项以工具箱回读的本地事实为证据；集成只面向本地目标。它是注册时的显式选择，不是只在本地的仓库的缺省，后者缺省绑定本地平台。之后绑定平台只是新增一份绑定与一类目标；Repo 身份、既有 ChangeSet Revision 与 Receipt 不变，进行中的意图沿用原绑定版本。

本地平台的绑定与外部平台一样逐项声明能力，按调研的实测结果填，不因为随包就默认全有。本批不随包带流水线执行器：本地平台绑定的「检查」能力声明为外部状态写回，来源只能是工具箱回读的本地测试事实，control 写回提交状态是记录、不单独构成证据；本地平台不能保证预期目标头，与已核对的外部平台相同。
...
代码协作平台不可用时，依赖平台当前回读的命令安全拒绝：远端合入、读评审请求状态、发布评审；不依赖它的操作继续：本地物化、封存、面向本地目标的集成。随包的本地平台停机时同样如此，不因为它在本机就放宽。全系统事实权威地图中的对应行见[系统边界](./system.md#全系统事实权威地图)。Receipt 的权威在账本，Git 里只有审计影子；平台丢失后不得凭 Git 提交重建 Receipt。
