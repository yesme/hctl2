...
外部步骤确认后，control 在同一事务激活 Repo；待确认 Repo 不接受 Project、Task 或 Run。同一命令重试复用原记录，不因确认丢失再建 Repo。主 Room 的建立与唯一性由 [Project 模块](./project.md#repo-注册与-project-归档)定义，注册本身不建任何 Room。参与者自己的 clone、共享对象库与工作树不成为 Repo 或 Project 的子对象，分责与必要引用见[系统边界](./system.md#repo-与执行现场)。
...
真正发布时再固定精确 ChangeSet Revision 与描述摘要。Result Proposal 只能提供策略允许的内容；换发布地点、换绑定或扩大发布范围都不在授权内，必须回到归属者重新预览与授权。策略中的「须人显式确认」开关随本次授权冻结；它的缺省是 Project 版本化设置的一部分（见 [Project 约束](./project.md#repo-注册与-project-归档)），本模块只引用其归属、执行时只读已冻结策略，Project 之后改默认值不影响已接受的调用。
...
本控制面的契约若事先声明接受他人、本控制面另一 Project 或其他控制面完成的精确平台集成事实，Repo 模块回读并核验平台目标、源版本、实际合入结果和证据，作为既有 Evidence 交给 Task；不是把平台标签或自述当凭证。squash/rebase 后的结果须按所选方式核对，不能只靠祖先关系。Task 不直接读平台，仍走自己的完成命令，其他验收项照旧。没有匹配本控制面意图时不补签 Integration Receipt；契约只接受本控制面凭证时，外部事实不够。契约事先声明与版本条件见 [Task 写入约束](./task.md#写入约束)。
