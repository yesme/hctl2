...
`hctl2-tool`（工具箱）是本模块的现场执行者：物化与隔离 Git 工作树、封存与保全 ChangeSet、执行面向本地目标的集成、回读 Git 事实与闭集外部机械事实。平台适配器是本模块经平台端口接入代码协作平台的适配代码：推送变更集分支、创建或更新评审请求、回读检查与评审状态、请求合并、回读评审评论正文。两者是分工：有平台的仓库照样由工具箱物化、封存与回读本地事实。
...
| 外部机械事实 | 工具箱回读的闭集事实（Snapshot/观测族）：提交的检查状态、评审请求是否合并、引用是否推进、路径与摘要 |
...
| 集成意图 / Integration Receipt | intent state version；待启动 / 结果未知 / 成功 / 失败；Receipt immutable | control 准入「合入 ChangeSet」命令；面向本地目标由工具箱执行与回读，面向远端目标由平台适配器投递与回读 | 同一 target ref 与所选授权形态下只允许一个获准结果；只有回读确认才能写 Receipt |
...
封存与准入是两件事：工具箱按有效租约把工作树内容封存成 Git 字节并回读，这是保存；账本接受这个版本，这是准入。顺序固定为：工具箱封存并回读；control 复核归属者状态、代次与租约仍然有效；Project 或 Run 准入 Result Proposal 的同一账本事务里，本模块准入 ChangeSet Revision。封存期间被取消或替代的归属者，其 Git 对象即使已经存在也不得成为获准版本；平台上出现一个提交也不是准入。没有归属者准入的封存只留审计。

评审对象对 {change_set_revision_id, change_set_id, parent_revision_id?, base_commit_sha, result_tree_sha} 使用[共享摘要规则](./system.md#命令与跨服务正确性)生成独立 review_subject_digest；它不是完整 ChangeSet Revision 的 revision_digest。评审身份由这五个字段决定：同一获准版本只换提交包装（内容与基线相同、提交对象不同）时身份不变；基线或结果树任一变化都是新 Revision，旧 Verdict 按 [Run 约束](./run.md#request重试与-gate)失效。结果树相同不是充分条件。
...
Harness 可以操作自己的 Git 工作树，但改写目标引用不产生 Receipt。下一次预览或回读只会显示预期目标头不匹配的分歧，由有权 actor 对账处理。工具箱校验 Git 基线、HEAD、tree、祖先关系与目标分支头；评审请求、检查与评审状态由平台适配器回读。
...
绑定声明「不能保证预期目标头」而 actor 未显式选择第二种形态时，集成意图拒绝。放行不是同一条命令的执行时降级，也不由回读结果倒推。
...
结果未知时，执行者回读 Git 与评审请求状态；收敛前不得签发成功 Receipt 或清理现场。远端写入被中断时同样按结果未知处理，并返回类型化恢复动作。
...
| 合并按钮、「已合并」通知 | 不是事前授权。有匹配的持久集成意图时供回读；没有时只作外部事实与分歧记录，不倒补意图，不补签 Receipt |
| 评审请求被改投另一目标 ref、被关闭或重开 | 只更新映射的状态部分并标为需要关注；不对错误目标写 Receipt |
| 直接改写受保护 ref 或绑定的评审请求分支 | 不支持的供应端修改：记录当前机械事实并标记分歧 |

映射到有权用户本人的平台动作要成为 human 命令请求，须满足系统边界要求的 actor、目标、前后版本与幂等依据，且绑定明确列出该动作；本约束当前不列出任何自动提交的平台动作，合入与发布都由 Workbench/CLI 的直接客户端连接提交。同一平台账号发出的多次批准映射不出多个席位；参与者是否各有平台身份由交付文档决定，不为此新建账号系统。
