...
HCTL 对已有工作的缺省移除入口是“取消并归档”：按本模块的取消前置提交命令，成功后从活动列表收起，保留历史与源卡片，不增加 Task 生命周期状态。未提交草稿可以丢弃。删除源卡片是另一个有权用户显式确认的 content 动作；预览按规范实体键列出源、卡片、不可逆后果，以及本控制面已知的全部绑定 Task（含各自的 Project、生命周期与活动 Run 引用及状态），说明删的是共享源卡，不是仅取消本 Project 的 Task；不声称列全其他控制面的使用。存在活动 Run 时须明确其处理选择；停止或取消仍分别经过原对象的授权与命令，删卡确认不替其他 Project 作决定。源删除确认未知时不报成功；确认删除后，各绑定分别追加 Snapshot/tombstone，保留 Task 与 Run，依赖当前回读的动作按原规则处理，不清除历史。
