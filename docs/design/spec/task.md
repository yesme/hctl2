...
**Task 依赖**：卡与卡之间的阻塞与父子关系是任务源的原生语义，归源持有；HCTL 认得它但不持有、不写回：适配器从源读取并投影到卡片与合并板，Task 记录不另存依赖对象，跨源的依赖不设计。它只在两处进入治理：「启动 Run」的预览对绑定 Task 的卡列出源上未关闭的阻塞方，等人显式确认后才启动，不自动拒绝；父卡的完成请求预览列出未完成的子卡，同样等人确认。源不提供依赖语义时投影为空，不阻拦。
