...
Project 入口同时固定控制面与 Project 引用，不能只凭 Repo 合并不同 Project。点击名称打开其唯一主 Room，点击“待你处理”标记打开[已有事项的投影](./project.md#repo-注册与-project-归档)；Rooms 只列该 Project 的 Topic Rooms，Kanbans 按已接入任务源分别进入，Runs 独立列出。跨列表引用保留原对象与 Project 归属；从本 Project 发起的关联与命令须核对目标归属，不以同 Repo 为由越过 Project 范围。

Run 列表保留等待、暂停与待人处理的非终态 Run，不仅列有施工者运行的条目。打开 Run 默认展示 DAG，可切换任务书；两种视图对应同一 Run，并列出当前施工者与具体步骤，支持并行节点。选择施工者后，经 Agency 已声明且本次获准的能力打开观察入口；无图形或终端输出不等于没有执行状态，关闭观察窗口不取消执行。展示分别引用执行、检查、评审、集成与 Task 验收的事实，未要求的环节不强加；缺证据、旧版本或结果未知不能显示为当前工作已通过。
