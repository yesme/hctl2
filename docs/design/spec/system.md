...
| Workflow 机械位置 | 通过绑定访问的 Workflow Engine | 已冻结的本地事实继续存在；Run 可按恢复合同失败/取消/替代收口，需要 fresh Engine readback 时不得正常完成 | 活动 Run 的机械位置按丢失/结果未知收口或显式替代；凭证链权威在 metadata 账本，审计影子在 Git |
