...
| Chat 端口绑定 | Room 到 chat server 房间的 Resolved Port Binding，外部 account/room stable IDs、获准身份映射与可选结构化 human 动作 allowlist（含经 homeserver 桥接接入的外部平台用户）；Room 的 content 家由它指认。准入前置见[Room 与消息](#room-与消息) |
