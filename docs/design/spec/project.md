...
| Chat 端口绑定 | Room 引用的 ResolvedPortBinding，及外部 account/thread stable IDs、成员映射、去重游标的字段组 |
...
Context 组装顺序固定为：显式引用 → 当前讨论窗口 → Project/Task/Run/Request → Git/Artifact/Receipt → 必需 Skill → 相关 Memo。RoomInvocation 的 ExecutionSpec 冻结最终 ContextManifest、逻辑参与者、Harness 候选、能力和权限；之后的 Room 消息不会偷偷改变已确认调用。
...
RoomInvocation 适合一次性的研究、比较或范围明确的写入。它可以持有一份 ExecutionSpec 和可选 Harness 运行时，但没有持久 DAG、候选自动切换、Gate 或自动后继；需要这些能力时应创建 [Run](./run.md)。
...
| HCTL 词 | 外部体系 | 一句话差异 |
| --- | --- | --- |
| Room | Matrix room / Slack channel | 语义基本一致：持久、多参与者、可桥接 |
| RoomEvent | Matrix event / Slack message | 与 Matrix 的只追加事件语义一致（编辑/撤回是新事件）；Slack 消息可原地编辑，桥接时按新事件或 tombstone 落账；HCTL 侧顺序由账本的 `room_sequence` 给出 |
...
| Chat 端口绑定 | Matrix bridge / Slack app 安装 | 差异：桥接只投递与回读，不拥有 Room 身份或历史 |
