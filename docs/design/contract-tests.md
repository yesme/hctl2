...
- 两个 Seat 各持合法的消费者 Bundle，投票时实际 Worker Profile 摘要与交付内容摘要均相同、法定票数为二：去重后只计一票，按法定票数不可达沿失败边推进，不签汇总凭证，两票留在裁决与审计记录；重复组内一票通过一票要求修改时不计通过票、按要求修改处理并标需要关注；声明了席位多样性而选定记录不满足时启动拒绝
...
- Workflow Revision 未声明关闭读回而「批准 Workflow」命令缺读回记录引用时拒绝；读回记录所引 Workflow Revision 摘要与本次批准不符、清单快照版本过旧、产出调用的 Bundle 含来源 Room 消息条目、产出调用带任何 Room 的消息读取权限、产出者在来源 Room 名册快照里、或声明读回须换 Worker Profile 而实际未换时都拒绝；声明关闭读回的批准照常通过；只改「上下文」章节的新 Workflow Revision 编译出的节点、边与席位与旧版本相同，多出节点或义务即失败
