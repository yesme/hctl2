...
- 两个 Seat 的 Worker Profile revision 与 Context Bundle 摘要均相同、法定票数为二时，双双通过 Gate 不得通过；两票都记入 Gate Receipt 且标明只计一票
- 外部执行节点无达成判据声明时编译拒绝；边未引用上游结晶或凭证标识时拒绝；图有环或边未落地时拒绝
- Workflow Revision 未声明关闭盲读回而「批准 Workflow」命令缺读回记录引用时拒绝；读回记录由在来源 Room 发过言的 Participant 产出时拒绝
- Gate Seat 的 Context Bundle 含其他 Seat 的裁决、作者身份或作者所用 Worker Profile 标识时拒绝交付
- Gate Receipt 缺逐票的 Worker Profile revision、Bundle 摘要、去重标记、证据通道等级或 token 估算时不得签发
...
- 三条逃生口——无契约卡、无 Run 路径、一票评审——各走一遍后，控制面存储里都有可回溯的对象与凭证，没有一条路径「走完什么都不记」
- 自举期间每条门的触发次数与绕过次数可查询；连续若干个真实变更里从未触发或全部被绕过的门要留下复审记录
