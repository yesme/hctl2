...
- Workflow Revision 未声明关闭读回而「批准 Workflow」命令缺读回记录引用时拒绝；读回记录所引 Workflow Revision 摘要与本次批准不符、清单快照版本过旧、产出调用的 Bundle 含来源 Room 消息条目、或产出者在来源 Room 名册快照里时都拒绝；声明关闭读回的批准照常通过
