...
- 未声明多样性时合法候选切换使两席同组按一票计；两席同配置而一席候选含不同配置并切换后成两票，之前不得提前判不可达；声明了席位多样性而选定记录或切换后不满足时拒绝；否决席无有效裁决而仍可产出时其余票已满不得签通过、不得撤销该席，已交提案不切换，候选耗尽按类型化技术失败或 Request 而不按通过或驳回处理，已取消或被替代的席位不因此再派工
...
- 登记时快照含三张清单以外的字段或条目种类时拒绝，直接文本来源记了提交者与内容摘要时通过；产出来源集合两次调用而读回用了其中一次的 Worker Profile 时拒绝；来源集合含调用 A（配置 X）、批准前最近一次调用 B（配置 Y）、读回用 Y 时通过；显式声明无模型产出调用时读回记录标注并通过；启用读回且声明了回避、来源缺失或引用无法解析时拒绝，补齐来源或撤回声明后通过；来源冻结后不得用变动的集合改变既有批准依据
