...
- **2026-09-06，决策史 §36（v0.17.1）。** 所有者拍板：只在本地的仓库缺省绑定随包的本地代码协作平台，选型 Gitea。本文「决定建议」第二条「其余平台暂缓」对外部平台维持；Gitea 从暂缓改为采用二进制，但身份是本地平台，不是「第二个外部平台」。对照表里 Gitea 一行的接口结论不变；调用面核对、运行形态与 tea 覆盖见[本地代码协作平台调研](./gitea.md)。
