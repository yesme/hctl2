...
> 深度标注：本轮是**链接级 web survey**，与 methodology-landscape 的逐仓库克隆审计不同级；其中四个样本（MyContext、LobeHub context-engine、First Tree Context Tree、Cumora triage 门）已在[实现证据](./README.md)完成源码级深审，直接引用其结论。值得升级为克隆审计的候选在文末列出。<br>
