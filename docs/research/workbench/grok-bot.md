...
- **2026-09-17**：所有者与 Codex 的方法论对照全文、五家 harness 的读后建议与主笔处置收在 `.memo/notes/grokbot-compare/`；落进设计正文的部分见台账 v0.18.4（长期主体是项目与承诺、协调靠制度不靠对话、伙伴走推荐层）。复用决策不变（仅参考行为）。对照里新核出可借鉴的行为：观察阶梯分状态、预览、接管三级；Skill 与 Routine 从成功过程沉淀；Bot 间私信在两端转录可见但处理轮次隐藏。已登记候选、未实施：按派工绑定的图形观察面。
