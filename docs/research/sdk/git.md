...
- 2026-09-05 #177 双审修正：临时索引先 `read-tree HEAD` 再 `add --all`，已跟踪但被忽略的文件进入快照；`ls-files --stage` 发现 `160000` 即拒绝保全拆除（嵌套仓库与 linked worktree 子模块）。`--confirm-discard` 绑定当前树 sha。输出拆成 `head_commit_sha`（快照父）与 `baseline_commit_sha`（基线标记）。残留清单加 `ignored_residue_total`。`reference-transaction` hook 注入覆盖 SALVAGE_UNPROVEN。研究正文未改。
- 2026-09-05 #177 复审修正：gitlink 只在磁盘上确实是仓库时才算存在（`<path>/.git` 在，或 `rev-parse --show-toplevel` 恰好等于该路径）；未初始化的已声明子模块不挡保全拆除。丢弃预览与 `removed_discarded` 输出带过滤后的 `nested_repositories`。研究正文未改。
- 2026-09-21 P2.2 戊复核：Repo 注册复用 `foundation::git` 的宿主二进制选择与环境清理，不引入 Git SDK。真实临时仓库测试覆盖空仓、detached HEAD、显式选择附注标签、独立副本与原 remote 保留、目标同名分支冲突、未选择的 refs 不推。首次推送使用 `push --atomic --force-with-lease=<ref>:`（空旧值，只准创建该 ref），随后 `ls-remote --refs` 对冻结 SHA 回读；目标已有不同 SHA 时拒绝，不把 `--force-with-lease` 当覆盖许可。附注标签保留 tag 对象本身，不剥成 commit；`.memo` / `.hctl2` 扫选定 refs 可达历史，HEAD 删除文件不等于历史不含它。调用边界与失败用例见 [Repo 实现说明](../../../src/crates/repo/README.md)。
- 2026-09-21 #287 双审复核（Grok 的凭据与 detached HEAD 两项）：注册使用的是控制面托管凭据，不同于工具箱沿用人的现场配置。凭据推送改从独立私有副本执行，初始化不带模板；`ls-remote` 不发现调用目录内的 Git 配置；所有注册 Git 子进程禁用 hooks、fsmonitor、全局/系统配置及 trace，原地 remote 更改不带凭据。复用 [Git 原生命令级 hooksPath](https://git-scm.com/docs/git-config#Documentation/git-config.txt-corehooksPath) 与 [配置/trace 环境变量](https://git-scm.com/docs/git#_environment_variables)，未改工具箱的用户配置策略。测试注入源 pre-push、URL rewrite、全局 hook、模板与 trace，验证未执行、未写追踪、目标 refs 正确。detached HEAD 与既有 `main` 不同则拒绝猜测，检出分支或显式选择 `refs/heads/main` 后再交付；不声称 Git 总有可发现的源仓库默认分支。
