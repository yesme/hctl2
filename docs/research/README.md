...
| [gitea.md](./gitea.md) | 随包的本地代码协作平台：Gitea v1.27.3 的调用面核对、运行形态与 tea 命令行覆盖 | E-SCM-GITEA | ⑥ 机械后端与基础设施 | 采用二进制（Gitea 与 tea 都随包），Forgejo 备选，Gogs 落选（没有评审请求接口，也不明显小）；适配器第一级用 tea，字段级操作走 `tea api` |
...
| [governance-attrition-20260908.md](./governance-attrition-20260908.md) | 治理功能为什么活不长：33 家的跨族归纳与对 HCTL2 的含义 | E-GOVERNANCE-ATTRITION | 跨候选归纳 | 仅参考行为；新建议五条进第三轮裁决 |
...
| [collective-intelligence-research-20260918.md](./collective-intelligence-research-20260918.md) | 人机集体智能（Collective Intelligence）综述：异质执行者、成型格式与可检验组织增益；把集体智能 / 分布式认知 / 协调理论接到 Room–Task–Run–Repo 边界。标题「群体智能」不是 Swarm | E-COLLECTIVE-INTELLIGENCE | 跨候选归纳 | 仅参考行为；文中记录格式与实验提案须另行评审才进规范 |
