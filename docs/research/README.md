...
| [gitea.md](./gitea.md) | 随包的本地代码协作平台：Gitea v1.27.3 的调用面核对、运行形态与 tea 命令行覆盖 | E-SCM-GITEA | ⑥ 机械后端与基础设施 | 采用二进制，Forgejo 备选，Gogs 落选（没有评审请求接口，也不明显小）；适配器第一级用 tea，字段级操作走 `tea api` |
