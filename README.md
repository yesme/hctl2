...
HCTL2 是把**人主导的目标塑形**与**机器驱动的可验证施工**连接起来的项目协作系统。它围绕代码仓库开展工作，每个 Project 分别组织自己的讨论、承诺与验收；协作与治理随用户走，默认部署在本地；面向同时使用多个 Coding Harness（Codex、Claude Code、OpenCode 这类编码代理工具，下称 Harness）的开发者。
...
> [!IMPORTANT]
> HCTL2 已进入早期实现，权威设计基线是 **草案 v0.18.11**。`src/` 现有 Rust 工作区与
> Linux x86_64、macOS arm64/x86_64 分目标依赖打包代码；三个目标均已通过原生整包生命周期验证，
> 但还没有可用的公共 CLI 或完整应用。

当前已有命令、离线安装包和四类打包依赖的具体操作方法见[HCTL2 使用说明](./docs/usage.md)。

讨论产品路径和结构时，先读[基础用户体验](./docs/user-experience/README.md#基础用户体验)：多单元用例、用户流程、术语纠正、project 导航及已定事项与接手清单已集中在这里。设计基线已按体验目录核对（G 批：方案 #262，落地 #264–#269，v0.18.8 到 v0.18.11）；差异与未实施能力见接手清单。
...
flowchart TB
    ...
    subgraph Control["控制面 · hctl2-control（五个领域模块）"]
        P["Project<br/>Room（聊天室）"]
        T["Task<br/>Kanban"]
        R["Run<br/>Workflow"]
        A["Participant<br/>Terminal"]
        C["Repo<br/>Change（变更）"]
    end
...
图中的 Room（聊天室）、Kanban、Workflow、Terminal 和 Change（变更）是五个模块对应的场景。Workbench、CLI 与 provider（供应端）原生客户端没有等级；动作按它落在哪个模块、带什么信封处理，分类规则见[系统约束](./docs/design/spec/system.md#客户端动作与-provider-事件)。图中各类单元可有多个实例、可跨机器连接：多个控制面可以复用同一个 Agency 与同一套内容系统，一个前端可以连多个控制面，见三面架构的[单元与连接](./docs/design/architecture.md#单元与连接)。

图里带 `hctl2-` 前缀的是本仓库交付的第一方组件；执行面的聊天、任务、工作流、代码协作平台四类系统由供应端承担——随包的本地实现或外接的外部服务，其中代码协作平台按仓库绑定：来自外部平台的仓库用外部平台，只在本地的仓库用随包的本地平台，`hctl2-tool` 是每个仓库都有的本地执行者；Agency 是参与者的供给方而不是模块，没有接入外部 Agency 时默认由发布包自带的本地参考实现承担；四类外部系统与本地运行时各选了谁、实现到什么阶段见[交付文档](./docs/design/delivery.md#实现阶段)。三面职责、场景与系统、供应商替换边界见[三面架构](./docs/design/architecture.md)；组件职责与控制面存储的精确划分见[系统边界的组件表](./docs/design/spec/system.md#组件)。

## 阅读入口

- **对齐用户体验或换机器接手讨论**：从[体验目录](./docs/user-experience/README.md#四份正文)进入，下一步见[已定事项与接手清单](./docs/user-experience/open-questions.md#待拍板与接手清单)。
- **新读者**：本页 → [愿景](./docs/design/vision.md) → [设计地图](./docs/design/README.md) → [三面架构](./docs/design/architecture.md)。
  ...
  - 最后看交付文档的[当前范围](./docs/design/delivery.md#当前范围)与[实现阶段](./docs/design/delivery.md#实现阶段)。
- **实现者**：从[约束层总则](./docs/design/spec/README.md)进入[系统边界](./docs/design/spec/system.md)、[连接约束](./docs/design/spec/connections.md)和对应的五份[模块约束](./docs/design/spec/README.md#文件)，再看[契约测试矩阵](./docs/design/contract-tests.md)。
