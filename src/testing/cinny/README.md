...
HCTL2 的 Chatroom 解决方案由 Tuwunel 服务端和 Cinny 浏览器客户端共同组成。发行构建会锁定、校验并打包 Cinny 官方 Web 发行包，以及 Static Web Server 官方单二进制；安装后由 `hctl2-services` 与其他依赖统一管理，不要求最终用户另外安装客户端、Python 或 Node.js。
...
- 使用官方 [Static Web Server `v2.44.0`](https://github.com/static-web-server/static-web-server/releases/tag/v2.44.0) 单二进制提供 loopback HTTP 服务；Linux 固定 musl 版本，macOS 固定对应 CPU 架构，三者均校验 SHA-256。
