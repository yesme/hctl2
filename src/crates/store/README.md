...
`Store::open_with_status` 取得 foundation 的 OS 锁后才开写连接。schema 1 只含身份，schema 2 加命令内核，schema 3 增加 outbox 的未发送取消状态；`user_version` 由 rusqlite_migration 管理。升级前用 Online Backup 保存可核验快照；升级失败恢复原 schema、身份和代次。升级期间 `StartupStatus::require_ready` 返回 `UPGRADE_IN_PROGRESS`，尚未服务返回 `STORE_NOT_READY`；乙负责并发呈现这些状态。
...
模块放弃尚未发送的动作时，可在同一命令事务调用 `cancel_pending_effect`：只准 `pending → cancelled`，不生成确认结果，释放该动作的冲突范围；`unknown` 拒绝取消。取消后不能重新投递或确认。schema 2 升级保留全部旧意图、状态及未决冲突范围。
