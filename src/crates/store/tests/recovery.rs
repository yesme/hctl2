...
fn old_schema_backup_is_verified_and_upgraded_before_restore_serves() {
    ...
    report.database_sha256 = foundation::bytes_sha256(&original);
    std::fs::write(
...
fn credentials_require_a_nonempty_secret_and_backups_preserve_only_references() {
    ...
    let mut store = temp.store();
    let secrets =
        foundation::SecretStore::user_file("hctl2-store-test", temp.path().join("secrets"));
    ...
    let absent =
        foundation::SecretStore::user_file("hctl2-store-test", temp.path().join("absent-secrets"));
    assert_code(
