...
错误对象三个字段是 `code`、`message`、`recovery_action`，与 `store` 对齐，不另定义 RPC 枚举。Git 领域正文仍是 JCS；本 crate 只承载进程间传输。`get` / `versions` 不是本服务的公共 Query。Subscribe 的 `snapshot` 本轮只标记需要重 Query，不携带投影。
