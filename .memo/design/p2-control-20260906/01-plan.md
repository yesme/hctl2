...
2. **账本 schema 怎么升级——所有者问「现在研究迁移是不是太早，能不能先假设账本不会迁移」。** 这里「迁移」撞了两个意思，先拆开。所有者说的迁移是大搬家：把底层 SQLite 换成云上的大数据库，配合大规模部署。这件事 P2 不做，约束层也没打算做——账本是一本用户级 SQLite（`spec/system.md` §控制面自己的存储），多用户与云队列都在交付文档的「明确不做」里；假设它不会发生是对的，几个月后真要做也是另一份计划。我说的迁移是小事：我们自己迭代产品时，账本的表结构一定会一直变——B0 建几张表，B1 加 Room 与 Task 的表，B2 加租约、意图、凭证的表，B3 之后每个 PR 都可能加一列。变了怎么办？要么删库重来（B1 之前账本里只有影子数据，删了不心疼），要么给账本一个版本号加一串按序编号的「从版本 N 到 N+1 的 SQL」，启动时补跑到最新——业界都是后一种，SQLite 官方给的版本号位置叫 `PRAGMA user_version`，Rust 里做这件事的小库叫 `rusqlite_migration`（专为我们已经在用的 rusqlite 写的，没有别的运行时）。**定**：B0 起就带这个版本号与按序 SQL 的机制，因为它几十行、以后省得回头补；不为「换数据库」做任何准备。0b 研究只是按纪律给这个小库落一页对象文件（版本、许可证、与 rusqlite 0.40 的兼容），不是一项研究课题。
3. **Repo 的稳定身份——所有者问为什么需要，「同一个 Repo 完全可以有多个 HCTL 展开工作，Repo 与 HCTL 是一对多」。** 一对多正是要它的原因。控制面归用户级（`architecture.md` §三个面）：你在 Mac 上一个控制面，Codex 的云端会话可能是另一个，将来合作者各有各的；同一个 Git 仓库被这些控制面各自登记时，它们得认出「我们说的是同一个仓库」，否则各家各发一个 ID，写进仓库里的东西（`<repo>/.hctl2/` 下的任务契约正文、施工图、备忘、策略）就对不上号，ChangeSet 与集成凭证里的仓库引用也各说各话。所以身份不能放在某一个控制面的账本里，只能放在仓库内容里跟着克隆走——这就是那个受跟踪文件 `<repo>/.hctl2/repo.toml`：一个稳定 ID 加共享配置的引用，任何克隆都带着它，工具箱 `repo inspect` 已经会读。它**不是**锁，也不是「这个仓库归哪个 HCTL」，每个控制面只是在自己的账本里记「我登记过它」；约束写的「Git 已存在获准身份时复用它」（`spec/repo.md` §Repo 注册与 Repo Instance 挂接）说的就是第二个、第三个控制面来登记时不再造新 ID。远端地址、目录名、HEAD 都不能当身份：会换、随便起、可能碰巧相同。有一个零成本的替代——用根提交的哈希当身份，不用写文件、不用提交；它的短处是 fork 与原仓库同根、分不开，而且带不了配置，约束选了文件（CT-REPO 里 fork 与身份碰撞要求确认）。所以留给 P2 的题只是「这个文件第一次由谁提交到哪个分支」：**定**默认分支上的普通文件，注册命令让工具箱在一个 ChangeSet 工作树里写出来，人经正常路径合入；合入前 Repo 保持待确认。
