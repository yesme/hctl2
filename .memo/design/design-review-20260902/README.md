...
> 状态：第三轮进行中——写法（#148）、约束（#149）、愿景与架构（#150）、Fable 写作与调研（#153）已合入 main（v0.16.4），两条补充裁决记入 #154；Codex 文档工具链（#156）与运行时及部件（#159）已完成，Grok 任务书三测试 PR 已开，任务书见 `30-r3-remaining-brief.md`。待所有者表态一处：I-03 的 112 句判为不做全量加词（理由见任务书一进度注），不同意可回清单逐句再议<br>
...
| 3.6 测试计划与对抗核验（Grok，任务书三） | 完成（文档工具链夹具 + Process Compose / wait / 五处库；结论在 #156/#159 评论） | `30-r3-remaining-brief.md` §任务书三 | 2026-09-04 |
...
- Grok 豁免表核验（任务书三）：`first_use_terms.allowlist` 46 条全部是规则豁免，不是「正文不该带对照」；其余五个检查器空表或专有名例外成立。结论写在 #156 评论，收缩豁免表是写作改动，不在本测试 PR。
