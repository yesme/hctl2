...
> 覆盖范围：§二–§五 的 Project 退场候选、单板入口、主 Room 加 Scoped Rooms 列表、Topic 强制回填及向导改名建议，已由新组织结构接替；不能整节宣称「逐步机械对照仍然成立」，也不能把同 Repo 多 Project 的工作范围一并作废，C2 已明确保留。异步状态、选人和观察分析仍可参考；显式不挂平台第三选项、模板跳过读回等增量仅为未采纳建议，不是本轮已定体验。
