...
> 状态：讨论中 · 部分被覆盖——所有者 2026-09-18 指出路径里的「project」混用了仓库与话题（四点澄清录在 `closure-control-repo-20260918.md` §一），本文第二节原来的闭包改述随之作废，以该文为准；第三节的逐步机械对照仍然成立；第五节的拍板项已并入同题备忘的清单<br>
> 基线：main @ `31ccffc`（草案 v0.18.6）<br>
> 去向：拍板清单以 `closure-control-repo-20260918.md` §九 与 `user-path-20260917.md` §六 为准；本文保留下来的独立增量见第五节末。本文只做对照与建议，不改 `docs/`
