...
> 状态：已落地 · P1 收口（2026-09-05，Fable）：甲 #174、乙 #177、丙 #176、丁 #179 全部合入，main @ `65ad69f`；`hctl2-tool` 六个子命令在三平台打包后端到端通过；P1 只备装，不宣称自举、无账本无 Receipt。下一份是 P2（control 与公共 CLI，B0 起），等所有者开；P2 开工前先看 `01-plan.md` §六<br>
...
| 丁 | 打包后三平台端到端、usage 与 README、状态板收口 | Grok | 完成 | PR #179：完整发行包 `complete-test` 跑通物化→提交→封存→集成→拆除与五类失败；双审测试合同已落实 | 2026-09-05 |
...
| #179 | [维持，两项测试修正](https://github.com/yesme/hctl2/pull/179#issuecomment-5552112049) | [维持，一项测试合同收紧](https://github.com/yesme/hctl2/pull/179#issuecomment-5552097819) | 陈旧锁先断言再删、测试 git 隔离全局配置、`recovery_action`/holder 钉在打包级 |
...
本轮已收口，这张表不再更新。接手 P2 的会话：先读 `01-plan.md` §六「延后与遗留」，P2 的计划另起目录，不在这里续写。
