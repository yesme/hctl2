...
> 状态：待读回与批准 · 所有者 2026-09-12 同意五项后由 Fable 编成；v2 按所有者意见把 §六 改成成品提示词（按批、节点、席位填好，PR 用分支名定位）；v3 按 Muse 读回改：N0 补已决来源、裁决到达设成节点 X 并定信号、再读回的触发与上限、C 批拆分的分叉规则、陪审席数写清、修正批的依据写清、缺席换人、版本戳以检查为准；第二次读回后补一句 X 与在飞节点的关系（措辞，不再读回）；批准前先读回（读回席位见 §三），所有者批准即合入；v4（2026-09-14）按所有者要求：C、B、D 三批全部提示词首行加用例警告，§二 加 D15–D18，B、D 的 P.1 加「用例哪一步」列<br>
...
**已知风险。** C 批最重，已按 #224 拍板第 10 条定为一个动手 PR，靠方案落点表逐行对照查漏；同一批 harness 连审五批会疲劳，所以每批只挑一个陪审（A 批多一个 Muse 试席）、读回固定由不在席位上的家做、缺席按 §五 第 7 条换人；所有者的裁决随时会来，来了按 §四 的节点 X 处理：在同一个 PR 里改方案或文档、改 05 §二 的表与本图。
...
| D15 | Agency 是参与者的代理：要人找 Agency，此后所有来往经 Agency 转接，客户端不直接连参与者；Agency 有全部通路、无治理权威；failover 只在冻结的执行规格内，阶梯为 Agency 内部自愈（无感）→ 控制面按冻结截止与取消处理 → 换 Agency 由人定；经转接的证据不得被改变，旧执行体回来封存前机械核图与席位 | 所有者 2026-09-13 原话（代理、内部 failover、人换 Agency、核图）与 2026-09-14「1 认」主审的三条边界与阶梯，原话见 #224「拍板 · C 批方案 · 补记」；05 §二 B 行 |
| D16 | 多写通则：别人的写入是事实，不是命令，也不是违规；它可以让本控制面重核冻结的前提、触发一次验收或请求；能不能当验收证据由本控制面的契约事先声明；批准和完成永远自己判；控制面之间不加锁。六个后端是五个单元（聊天、任务后端、工作流引擎、代码协作平台、Agency）加控制面存储，控制面存储的多写指多个前端同时操作同一控制面；本轮点名的实例三处：Repo 随 C、任务后端随 D、Agency 随 B，其余后端有实例时随各自批 | 所有者 2026-09-14「2 认」与后端个数原话，见 #224「拍板 · C 批方案 · 补记」；拍板评论第 3 条 |
| D17 | 施工图 lint 加去重检查：所有者原话「同个 worker profile、拿相同的交付内容，法定票只算一票；这种检查应该在 DAG 施工图 lint 的时候 mechanical 地检验出来」；去重键沿 R 批现行，lint 时能静态判的部分由 D 方案定，算不出的仍由 R 批启动拒绝兜底，不造第二套上界 | 所有者 2026-09-13 原话，见 #224「拍板 · C 批方案 · 补记」；05 §二 D 行 |
| D18 | 用例优先：每处改法、每个待裁项先在用例里找到那一步，人已显式声明的不成题，用例外的标来源；三批全部提示词首行带这句 | 所有者 2026-09-13（C 批 C1 撤题）；05 §四 |
| D19 | Agency 是唯一通路且功能不泄露（含代次）；控制面不需要知道 Agency 内部换人；多租户隔离（每个配对的控制面一个隔离租户）；待命是 Agency 实现细节不暴露；Gate 多样性只留 Worker Profile 互异与回避作者；B 一个动手 PR | 所有者 2026-09-14（#228「裁决 · B 批 · 第一次」） |

**尚未定形**（在对应批次的方案里定，不进图）：P2.1 何时开工。（F1/R1/R3 随 R 批 #220 拍板、§37 行文随 A 批 #223 已定，2026-09-14 删；B 批待命随 #228 裁决 4 定为 Agency 内部实现、产品要求落交付文档，2026-09-14 删；D 批 lint 哪部分能静态判随 #230 拍板乙已定，2026-09-16 删。）

**出界**（不在本图）：P2 实现工作包与署名关卡的实现；Repo 身份长聊（C 批拍板撤题，不再是题，#224）；验证器二级、三级（随 P2.1）；把七问整理成技能（05 §十 待办）。
...
| D 看板、Run 交接与交付 | Fable | Codex | Grok | GLM | Muse（Gemini 不可用） | Grok（原 K3；所有者 2026-09-14：K3 周配额将尽，D 开工时贴「席位变更」） |
...
X 不在链上，它插在任何两个节点之间；点火它不改变其他节点的判据，只改变它们要对照的方案或图：正在进行中的节点不回退重判，X 完成后才进入的节点按改后的图走。C 批已按 #224 拍板第 10 条不拆，版本表维持 §三。

**修正批没有 P 子图**：它的 W.1 依据不是方案 PR，而是 §三 表里写的两处裁决清单（#216 报告修订节与 05 §二「修」行）。

**C 批不拆**（#224「拍板 · C 批方案」第 10 条）：一个动手 PR、一个 W 子图、v0.18.1，提交组织由作者决定；W.1 的 PR 描述按方案 §六落点表逐行写清实际落点，W.2 按该表查漏。B、D 版本与依赖维持 §三及本图，不启用第二动手分支。
...
| D P.1–P.7 | 完成 · 三家独审（GLM 改后可进、Grok、Codex 修正后再拍板）、交叉一轮（L1 撤「静态上界」、X1 撤「持有版本的 Agency」通用规则）、Muse 读回一次只改措辞，所有者 2026-09-15 拍板（L1 乙、E1 甲、换卡不做、一个 PR），方案 v5 随 #230 合入；「席位变更」核对席 K3→Grok 已贴 | [13-batch-d-plan.md](./13-batch-d-plan.md)；#230 |
| D W.1 | 完成 · 合入 `688bdc1`（v0.18.3）；PR 描述按 13 §六 落点表逐行对照；含研究层两份复核记录与 P2 计划补记 | #231 |
| D W.2–W.4 | 完成 · W.2 Grok「可合」+ Codex「修正后可合」七条，W.3 修正 `e557647`；合并后核对（Grok，#231 评论）：无回退，三处 Board 旧词残留随收口 PR 改口 | #231 |

改写 DAG 至此走完（2026-09-16）：修正批、R、A、C、B、D 六批全部合入，基线 v0.18.3。剩下的是 [05 §十](./05-rewrite-process.md#十待办) 的待办：把七问与方案形状整理成技能；P2.1 何时开工由所有者定。

改写 DAG 之后（2026-09-17）：

| 节点 | 状态 | 出处 |
| --- | --- | --- |
| 05 §十 待办：七问与方案形状进技能 | 完成 · 评审技能加「方案批的规矩」，写作指南 S4；GLM + Codex 轻审后合入 `725ee27`；同 PR 追加陪审团三层审（所有者 09-17） | #239 |
| 研究复核：Gitea/tea 本机运行验证、GitHub 沙箱写侧验证 | 完成 · Grok + Codex 轻审后合入 `202cb72` | #240 |
| E 批：delivery 七条未决裁决落地（v0.18.5） | 完成 · 所有者逐条拍（1、5 删；2、6 安全原则进 CONSTRAINTS；3 依赖归源投影、只有阻塞进启动预览；4 甲 14 天；7 甲）；Grok + Codex 轻审后合入 `2ab352d`；因 #245 对照收口先到 v0.18.4 而改号 | #243 |
| S 批：安全策略面（v0.18.6） | 完成 · 所有者「安全那个也改。开 PR」；GLM + Codex 轻审后合入 `31ccffc` | #244 |
| F 批：Project 与 Task 解耦 | 已归档 · 方案 v3 四席交叉过、待拍板时，所有者 09-19 的固定归属与独立 Namespace（#257，v0.18.7）取代了树状图前提；K0–K9 与待裁 a–d 全部失效，方案不合入，memo 留档并写明覆盖范围；复盘两条（口头新前提先过用例再开批；造成症状的旧规则要进备选表）随 G 批带进评审技能 | #246、[15-batch-f-plan](./15-batch-f-plan.md) |
| G 批：全库对照 v0.18.7 核对 | 完成 · 方案 v3 四席三层审加交叉轮，所有者 09-19 裁 A 档五项（全按 Codex 选法）、B 档无反对；动手六个 PR 当日全部合入（#266 Codex 机械族；#264 v0.18.8；#265 v0.18.9；#267 v0.18.10；#268 v0.18.11；#269 小尾巴），合并后核对 Grok；留待：A4 十行 CT 另立小批、B8 扫描扩围先 dry-run、#263 归纳机制单独出方案 | #262、[16-batch-g-plan](./16-batch-g-plan.md) §九 |
...
先读所有者的用例 `.memo/notes/HCTL_case_study.md`。方案里的每处改法、每个待裁项，先在用例里找到对应的那一步，写明系统在这一步要决定什么、依据是人已显式声明的还是推断的；人已显式声明的不成题；用例里没有的情形标「用例外」并写来源，不得想当然。
再读 PR #224 上的「拍板 · C 批方案 · 补记：聊天原话与出处」：所有者 2026-09-13/14 的裁决原话（Agency 代理模型与 failover 阶梯、多写通则与六个后端、施工图 lint 去重检查、用例优先），08 §二 D15–D18 引的就是它。
你在 yesme/hctl2 轻审 C 批（Repo 与治理正文） 的动手 PR：`gh pr list --head codex/batch-c --state open` 找到的那一个 PR。拍板的方案：`.memo/design/case-study-20260907/11-batch-c-plan.md`（合入 main 的版本）与所有者的评论「拍板 · C 批方案」。你的席位：轻审（Fable）。查漏按方案 §六 落点表逐行对照 PR 描述（拍板第 10 条）。
先 git fetch origin，读 AGENTS.md、CONSTRAINTS.md。
...
先读所有者的用例 `.memo/notes/HCTL_case_study.md`。方案里的每处改法、每个待裁项，先在用例里找到对应的那一步，写明系统在这一步要决定什么、依据是人已显式声明的还是推断的；人已显式声明的不成题；用例里没有的情形标「用例外」并写来源，不得想当然。
再读 PR #224 上的「拍板 · C 批方案 · 补记：聊天原话与出处」：所有者 2026-09-13/14 的裁决原话（Agency 代理模型与 failover 阶梯、多写通则与六个后端、施工图 lint 去重检查、用例优先），08 §二 D15–D18 引的就是它。
你在 yesme/hctl2 轻审 C 批（Repo 与治理正文） 的动手 PR：`gh pr list --head codex/batch-c --state open` 找到的那一个 PR。拍板的方案：`.memo/design/case-study-20260907/11-batch-c-plan.md`（合入 main 的版本）与所有者的评论「拍板 · C 批方案」。你的席位：轻审（Grok）。查漏按方案 §六 落点表逐行对照 PR 描述（拍板第 10 条）。
先 git fetch origin，读 AGENTS.md、CONSTRAINTS.md。
只查三样：一、改出来的文字是否忠于拍板的方案——逐处对，多改的、漏改的、改走样的各列一张小表；二、每句话的层对不对（愿景 / 架构 / 约束 / 交付 / 实现），实现名与约束用语有没有漏进上一层；三、约束改了的地方配没配契约测试用例，用例能不能失败。版本戳应为 v0.18.1，全库一致。
再跑 cd src && ./buck2 test root//build/docs/...，写结果。
...
分支 `claude/batch-b-plan`，开 PR、不自合，描述按模板三节；提交信息末尾不放 harness 会话链接。方案 §一 的范围表加一列「用例哪一步」，用例外的情形单列并写来源。
```
P.3 作者说明与 v2（Fable）
...
分支 `claude/batch-d-plan`，开 PR、不自合，描述按模板三节；提交信息末尾不放 harness 会话链接。方案 §一 的范围表加一列「用例哪一步」，用例外的情形单列并写来源。
```
P.3 作者说明与 v2（Fable）
...
不改文件，不开 PR，不 push，不合并。
```
#### W.4 合并后核对（Grok，原 K3；所有者 2026-09-14 席位变更）
...
先 git fetch origin，读 AGENTS.md、CONSTRAINTS.md。按指定提交读文件（git show 提交:路径），不以本地工作树为准。
只用四种情况的清单，对本批改动过的文件逐个核：一、已经改对、后来又改坏（尤其合并冲突处置把全库换词批改回去）；二、新规则与保留的旧规则不一致；三、已明确安排后续批次、尚未实施（只登记，不算问题）；四、所有者有意改变了原来的决定（不报作回退）。另核 05 §二 表与 08 进度表是否随本批更新。
产出一条评论，标题「Grok · D 批 · 合并后核对」，第一句写"无回退 / 有回退（n 处）"；每处写位置、证据（提交）、四种里的哪一种、建议。纸面反例不等于运行故障；CI 通过不等于自然语言一致。
不改文件，不开 PR，不 push。
```
### P.7 · 所有者的拍板评论（格式）
