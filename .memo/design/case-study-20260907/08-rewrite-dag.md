...
| D15 | Agency 是参与者的代理：要人找 Agency，此后所有来往经 Agency 转接，客户端不直接连参与者；Agency 有全部通路、无治理权威；failover 只在冻结的执行规格内，阶梯为 Agency 内部自愈（无感）→ 控制面按冻结截止与取消处理 → 换 Agency 由人定；经转接的证据不得被改变，旧执行体回来封存前机械核图与席位 | 所有者 2026-09-14 裁定（#224 拍板评论第 11 条；05 §二 B 行） |
| D16 | 多写通则：别人的写入是事实，不是命令，也不是违规；它可以让本控制面重核冻结的前提、触发一次验收或请求；能不能当验收证据由本控制面的契约事先声明；批准和完成永远自己判；控制面之间不加锁。六个后端（聊天、任务后端、工作流引擎、代码协作平台、Agency、控制面存储）各落自己的实例 | 所有者 2026-09-14 裁定（#224 拍板评论第 3 条） |
| D17 | 施工图 lint 加去重上界：Gate 按席位 Worker Profile 去重后的通过票上界少于法定票数，登记/编译时拒绝 | 所有者 2026-09-14 裁定；05 §二 D 行 |
| D18 | 用例优先：每处改法、每个待裁项先在用例里找到那一步，人已显式声明的不成题，用例外的标来源；三批全部提示词首行带这句 | 所有者 2026-09-13（C 批 C1 撤题）；05 §四 |

**尚未定形**（在对应批次的方案里定，不进图）：F1 的具体结算规则（等组稳定还是关闸时比较）；R1 清单快照的冻结来源；R3 无产出调用时回避策略的适用；B 批待命与会话复用写不写进设计正文；A 批决策史 §37 的行文；P2.1 何时开工。
