...
> 状态：讨论中 · 两轮审阅完成；§九 三处里用例修改与"账本"禁用已落（#195、#194），第 13 条治理正文已拍板（`03-governance-text.md` v3，五家一致，所有者授权收口）；第 13 条第十节按 Codex 修正后合入（#197）；用例的机械化验证器提案在 `04-scenario-validator.md`（不自合）；下一步开"设计文档怎么改"<br>
...
| `04-scenario-validator.md` | Fable | 用例怎么变成机械化验证器：参考用例作为第零级产物（编号、落点、十六条不变量），一级文档不变量检查、二级一台机器上的多单元模拟、三级真机；结构化脚本草稿；提案，待所有者与 Codex 看 |
