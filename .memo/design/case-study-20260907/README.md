...
| `06-batch-a0-vocabulary.md` | Fable | A0 词汇批的方案（v2）：五种改写情境加判别顺序、例外表、词族并入、动词"记账"剔出、前端定名、机械检查三处补、v0.17.2 与台账、留给 C 批回头的句子、第二轮请核的分歧点 |
| `07-repo-instance-question.md` | Fable | Repo Instance 去留（v3，按 Codex 与所有者讨论后的收窄）：不作为必备架构抽象，也不改名成现场引用靠分散字段维护；按操作各留必要引用（派工认版本与权限、交付认成果与来源、更新目标认目标）；约束层只写条件不写机制；受限路径与本机克隆维持 |
