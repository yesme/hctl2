...
| `04-scenario-validator.md` | Fable | 用例怎么变成机械化验证器（v2，按 Codex 修正）：参考用例作为第零级产物（编号、落点、不变量各带前提），一级文档关联与已裁定词汇的检查、二级同机多单元行为测试（留执行历史、反向用例、假供应端不超能力）、三级真机；脚本草稿；不新增 CT 族 |
