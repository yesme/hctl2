...
> 主备忘：`workbench-shell-reopen-20260826.md`。本记录回填主备忘要求的 Ubuntu 物理机一次性探针，测试时仓库基线为 main @ `314f6fc`。探针源码、构建目录与原始日志位于本机磁盘暂存目录 `~/tmp/hctl2-probes/`，不作为产品源码提交；本文件保留可复核的版本、结果、测量口径、产物摘要和上游反馈链接。
