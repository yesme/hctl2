...
> 日期：2026-09-06<br>
> 说明：Informative · 针对用户多机多现场案例（`HCTL_case_study.md`）的架构裁决、拓扑分析，以及所有者选型定界记录。<br>
> 修订：2026-09-07 按 PR #193 的讨论改口三句（身份写入 Git、会话是消耗品、必须依赖公共 Remote），作者 Antigravity 已逐句认可，见 `.memo/design/case-study-20260907/01-unit-model.md` §八。
...
1. **活跃进程不跨机迁移**：物理 Harness 进程和 PTY 随时可能因网络或宿主异常而断开，跨机维护透明的进程迁移在系统级极度脆弱、成本高昂且收益低；但未封存的修改和会话记录不是可随意丢弃的消耗品，只有已保全的字节才谈得上重建；
...
- **演进意义**：
  - Repo 的身份以平台上的仓库身份为准，Git 根提交作旁证，独立于任何单一 Project 或 Control；
  - 多个 Control 实例各自登记同一个仓库时，靠平台身份认出这是同一个仓库；
...
1. **临时物理工作台 vs 永久逻辑案卷**：Worktree 是文件系统中的具体检出文件夹，是可以重新物化的物理资源，但其中未封存的修改不可重建，雇佣期间也不必每次封存都拆；ChangeSet 是控制面账本和 Git 历史中登记的逻辑变更提案案卷。
2. **一对多的版本演进**：同一个 ChangeSet 在经历多次返工演进（Revision 1 → 2 → 3）时，物理 Worktree 可以在封存并保全之后被拆除（`hctl2-tool archive remove`），返工在封存它的那台机器上用原工作树或重新物化（`worktree materialize`）。物理目录的路径甚至操作系统都可改变，但 ChangeSet 身份保持唯一与连续。
...
2. **Git 对象的汇聚依赖**：不同机器上的 Git 对象库是物理隔离的。跨机派工的改动若要最终集成，要有一条可达的交付路径：共同的协作平台（GitHub/GitLab，或随包的本地平台）是一种，持有凭据的单元中转是另一种；没有任何交付路径的仓库，派工范围才受限于承载该 Repo Instance 的本机 Agency。
