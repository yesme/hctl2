...
- 参与者不响应而控制面还活着（Room 里 @ 了没反应、Run 里超时）：这是 Agency 超时与 failover 的问题。控制面通知原 Agency，由它内部 failover——重启失联的参与者，或改派一个备份——从控制面看无感。Agency 是参与者的代理：要人直接找 Agency，之后与参与者的全部交互经 Agency 转接，客户端也不直接与参与者沟通。Agency 自己也挂了，要由人指定别的 Agency 提供备份，涉及成本与授权，程序不能机械恢复。死掉的参与者回来时，要机械地核对施工图还是那张图、自己还是那个席位，不是就不能 check-in；用 Git 的 check-in 前置钩子做，不靠散文。（所有者 2026-09-13 补）
