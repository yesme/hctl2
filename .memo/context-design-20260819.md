...
## 19. 提案：Room 的"前情提要"（2026-08-24，待拍板）
...
待拍板点：**"没配 small-brain 就没有滚动纪要"的缺省够不够用**。长房间里未结晶的口头共识可能被漏掉；备选是允许主模型偶尔生成纪要（破坏"组装不花大模型 token"）。倾向不破——重要结论就该结晶成 Memo，缺省态的缺陷反而推动显式沉淀。
