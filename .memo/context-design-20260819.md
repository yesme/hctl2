...
裁决（2026-08-24 所有者拍板）：按提案通过。缺省态（无 small-brain）不生成滚动纪要、只用近详远略裁剪，不为此破坏"组装不花大模型 token"——重要结论就该结晶成 Memo，缺省态的缺陷是故意保留的沉淀压力。设计层正文已落 docs/design/context.md「前情提要」一节；合同落点（纪要作为派生缓存的清单引用字段、提升预览预填）随 Context 合同裁决轮进 spec。
